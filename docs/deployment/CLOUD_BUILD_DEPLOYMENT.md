...
2. **Rollback**: Deploy a previous revision.
   ...
   ```bash
   gcloud run deploy emotion-detection-api --image us-central1-docker.pkg.dev/$PROJECT_ID/samo-dl-repo/emotion-detection-api:OLD_BUILD_ID --region us-central1
   ```
