...
from ..src.security_setup import get_environment, setup_security_middleware
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("secure_api_server.log"), logging.StreamHandler()],
)
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if not allowed:
                ...
                update_metrics(
                    response_time, success=False, error_type="rate_limited", rate_limited=True
                )
...
class SecureEmotionDetectionModel:
    def __init__(self):
        ...
        self.model_path = (
            Path(env_model_dir).expanduser().resolve() if env_model_dir else default_model_dir
        )
        ...
        if not all(p.exists() for p in required_all):
            logger.warning("Secure model directory lacks expected files. Running in stub mode.")
            self.tokenizer = None
            self.model = None
            self.loaded = False
            return
    ...
    def predict(self, text, confidence_threshold=None):
        ...
        try:
            ...
            inputs = self.tokenizer(
                sanitized_text, return_tensors="pt", truncation=True, padding=True, max_length=512
            )
...
def _extract_and_filter_texts_or_raise(data: Dict[str, Any]) -> Tuple[List[str], List[str], int]:
    ...
    if not data or "texts" not in data or not isinstance(data["texts"], list):
        raise _ClientError('Field "texts" must be a list of strings', 400, "validation_error")
...
def _validate_alignment_count_or_raise(results: Any, expected_count: int) -> bool:
...
def _build_single_response(
    sanitized_text: str, dist: List[Dict[str, Any]], warnings: List[Any]
) -> Dict[str, Any]:
...
def predict():
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Validation error (%s) from %s", e.__class__.__name__, request.remote_addr
            )
        ...
        result = model_instance.predict(
            sanitized_data["text"], confidence_threshold=sanitized_data.get("confidence_threshold")
        )
...
def predict_batch():
    ...
    try:
        ...
        try:
            ...
            update_metrics(response_time, success=False, error_type="validation_error")
            logger.warning(
                "Batch validation error (%s) from %s", e.__class__.__name__, request.remote_addr
            )
        ...
        for text in sanitized_data["texts"]:
            if text.strip():
                result = model_instance.predict(
                    text, confidence_threshold=sanitized_data.get("confidence_threshold")
                )
                results.append(result)

        response_time = time.time() - start_time
        update_metrics(response_time, success=True, sanitization_warnings=len(warnings))
...
def nlp_emotion():
    ...
    try:
        ...
        try:
            ...
            update_metrics(
                time.time() - start_time, success=True, sanitization_warnings=len(warnings)
            )
        ...
        return jsonify(
            {
                "server_metrics": {
                    ...
                    / max((datetime.now() - metrics["start_time"]).total_seconds() / 60, 1),
                },
