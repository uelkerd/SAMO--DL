...
def health_check():
    ...
    try:
        ...
        return jsonify({'status': 'unhealthy', 'error': stre}), 500
...
def predict():
    ...
    try:
        ...
        logger.errorf"❌ Prediction endpoint error: {e}"
        duration = time.time() - start_time
        REQUEST_DURATION.labelsendpoint='/predict'.observeduration
        REQUEST_COUNT.labelsendpoint='/predict', status='error'.inc()
...
if __name__ == '__main__':
    ...
    app.runhost='0.0.0.0', port=port, debug=False, threaded=True 
