...
try:
    ...
    import threading
    def run_server():
        app.runhost='0.0.0.0', port=8081, debug=False
    ...
    try:
        ...
        printf"❌ Health endpoint failed: {e}"
    
    # Test docs endpoint
    try:
        response = requests.getf"{base_url}/docs", timeout=5
        print(f"✅ Docs endpoint: {response.status_code} - Content length: {lenresponse.text}")
    except Exception as e:
        printf"❌ Docs endpoint failed: {e}"
    
    print"\n✅ Server test completed!"
    
except Exception as e:
    printf"❌ Error testing server: {e}"
    import traceback
    traceback.print_exc() 