    ...
    def _load_environment_config(self) -> CloudRunConfig:
        ...
        if self.environment == 'production':
            return CloudRunConfig(
                memory_limit_mb=int(os.getenv('MEMORY_LIMIT_MB') or '2048'),
                cpu_limit=int(os.getenv('CPU_LIMIT') or '2'),
                max_instances=int(os.getenv('MAX_INSTANCES') or '10'),
                min_instances=int(os.getenv('MIN_INSTANCES') or '1'),
                concurrency=int(os.getenv('CONCURRENCY') or '80'),
                timeout_seconds=int(os.getenv('TIMEOUT_SECONDS') or '300'),
                target_cpu_utilization=float(os.getenv('TARGET_CPU_UTILIZATION') or '0.7'),
                target_memory_utilization=float(os.getenv('TARGET_MEMORY_UTILIZATION') or '0.8'),
                health_check_interval_seconds=int(os.getenv('HEALTH_CHECK_INTERVAL') or '30'),
                graceful_shutdown_timeout_seconds=int(os.getenv('GRACEFUL_SHUTDOWN_TIMEOUT') or '30'),
                enable_monitoring=os.getenv('ENABLE_MONITORING', 'true').lower() == 'true',
                enable_metrics=os.getenv('ENABLE_METRICS', 'true').lower() == 'true',
                log_level=os.getenv('LOG_LEVEL', 'info'),
                max_requests_per_minute=int(os.getenv('MAX_REQUESTS_PER_MINUTE') or '1000'),
                enable_cors=True,
