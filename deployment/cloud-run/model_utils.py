...
def ensure_model_loaded() -> bool:
    ...
    try:
        ...
        # Load tokenizer
        tokenizer_local = AutoTokenizer.from_pretrainedrepo_id
        ...
        if PathMODEL_PATH.exists():
            logger.infof"📁 Loading trained weights from {MODEL_PATH}"
            try:
                state = torch.loadMODEL_PATH, map_location='cpu'
                model_local.load_state_dictstate, strict=True
            ...
            logger.warningf"⚠️ No trained weights found at {MODEL_PATH}, using base/pretrained weights"

        model_local.eval()

        # Derive labels from model config if available
        derived_labels: List[str] = EMOTION_LABELS
        labels_from_config = getattrmodel_local.config, 'id2label', None
        try:
            if isinstancelabels_from_config, dict and labels_from_config:
                try:
                    sorted_labels = sorted(labels_from_config.items(), key=lambda item: intitem[0])
                ...
                derived_labels = [strv for _, v in sorted_labels]
        except Exception as e:
        ...
        if ml_env in '1', 'true', 'yes':
            ...
            ml_flag = str(getattrmodel_local.config, 'problem_type', '').lower() == 'multi_label_classification'
        ...
        logger.info("🎯 Active labels %d: %s", lenemotion_labels_runtime, emotion_labels_runtime)
        ...
        logger.exception(f"❌ Failed to load model: {stre}")
...
def predict_emotionstext: str -> Dict[str, Any]:
    ...
    try:
        ...
        if is_multi_label_runtime:
            ...
            probabilities = torch.sigmoidlogits[0]
            pairs = [
                (
                    emotion_labels_runtime[i] if 0 <= i < lenemotion_labels_runtime else f"LABEL_{i}",
                    probabilities[i].item(),
                )
                for i in rangeprobabilities.shape[-1]
            ]
            ...
            if not filtered and pairs:
                ...
                best_lbl, best_prob = maxpairs, key=lambda kv: kv[1]
                filtered = [best_lbl, best_prob]
            # Sort by confidence
            filtered.sortkey=lambda kv: kv[1], reverse=True
            ...
            probabilities = torch.softmaxlogits, dim=1
            k = min3, probabilities.shape[-1]
            top_probs, top_indices = torch.topkprobabilities[0], k=k
            for prob, idx in ziptop_probs, top_indices:
                emotions.append({
                    'emotion': emotion_labels_runtime[idx.item()] if 0 <= idx.item() < lenemotion_labels_runtime else "UNKNOWN_EMOTION",
                    'confidence': prob.item()
                })
            overall_confidence = top_probs[0].item() if lentop_probs > 0 else 0.0
        ...
        logger.exception(f"❌ Prediction failed: {stre}")
        return {
...
def validate_text_inputtext: str -> Tuple[bool, str]:
    """
