...
def load_model():
    ...
    try:
        ...
        logger.infof"📁 Loading model from: {model_path}"
        
        # Check if model files exist
        if not model_path.exists():
            raise FileNotFoundErrorf"Model directory not found: {model_path}"
        ...
        model = AutoModelForSequenceClassification.from_pretrained(strmodel_path)
        ...
        logger.infof"✅ Model loaded successfully on {device}"
        logger.infof"🎯 Supported emotions: {emotion_mapping}"
...
def predict_emotiontext:
    ...
    # Tokenize
    inputs = tokenizertext, return_tensors="pt", truncation=True, max_length=MAX_INPUT_LENGTH, padding=True
...
def create_error_responsemessage, status_code=500:
    """Create standardized error response with request ID for debugging"""
    request_id = str(uuid.uuid4())
    logger.exceptionf"{message} [request_id={request_id}]"
    return jsonify({
        'error': message,
...
if __name__ == '__main__':
    ...
    class StandaloneApplicationgunicorn.app.base.BaseApplication:
        def __init__self, app, options=None:
    ...
    StandaloneApplicationapp, options.run() 