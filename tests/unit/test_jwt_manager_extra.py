...
def test_create_token_pair_structure():
    ...
    token_pair_dict = token_pair.dict(
                                      ) if hasattr(token_pair,
                                      "dict") else dict(token_pair
                                     )
    assert set(
               token_pair_dict.keys()) == {"access_token",
               "refresh_token",
               "token_type",
               "expires_in"}
...
def test_refresh_access_token_success_and_failure():
    ...
    # Expired refresh token should fail (
                                         re-sign with manager's secret to keep signature valid
                                        )
    import jwt
    payload = jwt.decode(
                         refresh,
