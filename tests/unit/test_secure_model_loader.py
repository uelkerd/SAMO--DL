...
class TestSandboxExecutor(unittest.TestCase):
    ...
    def setUp(self):
        self.executor = SandboxExecutor(
            max_memory_mb=512, max_cpu_time=10, max_wall_time=20
        )
    ...
    def test_load_model_safely(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pt", delete=False) as f:
            model = TestModel()
            torch.save(
                {"state_dict": model.state_dict(), "config": {"model_name": "test"}},
                f.name,
            )
...
class TestSecureModelLoaderIntegration(unittest.TestCase):
    ...
    def test_corrupted_model_file_handling(self):
        ...
        torch.save(
            {"state_dict": suspicious_state_dict, "config": self.test_config},
            tampered_model_file,
        )
