...
class TestSecurityIntegration(unittest.TestCase):
    ...
    def test_comprehensive_security_headers(self):
        ...
        for header, validation in required_headers:
            ...
            if validation == "non-empty":
                self.assertGreater(
                    len(response.headers[header]), 0, f"Header {header} should not be empty"
                )
    ...
    def test_csp_policy_upgrade_insecure_requests(self):
        ...
        csp_policy = self.middleware._build_csp_policy()
        self.assertIn(
            "upgrade-insecure-requests", csp_policy, "Missing upgrade-insecure-requests directive"
        )
    ...
    def test_csp_policy_block_mixed_content(self):
        ...
        csp_policy = self.middleware._build_csp_policy()
        self.assertIn(
            "block-all-mixed-content", csp_policy, "Missing block-all-mixed-content directive"
        )
    ...
    def test_permissions_policy_encrypted_media(self):
        ...
        permissions_policy = self.middleware._build_permissions_policy()
        self.assertIn(
            "encrypted-media=()", permissions_policy, "Missing encrypted media restriction"
        )
    ...
    def test_suspicious_pattern_detection(self):
        ...
        with self.app.test_request_context(
            "/test", headers={"X-Forwarded-Host": "malicious.com", "User-Agent": "sqlmap/1.0"}
        ):
    ...
    def test_production_security_headers(self):
        ...
        self.assertEqual(
            response.headers["X-Frame-Options"], "DENY", "X-Frame-Options should be DENY"
        )

        self.assertIn(
            "X-Content-Type-Options", response.headers, "Missing X-Content-Type-Options header"
        )
        self.assertEqual(
    ...
    def test_ua_blocking_returns_403(self):
        ...
        with self.app.test_request_context(
            "/blocked", headers={"User-Agent": "sqlmap/1.0 curl/7.88 nikto/2.1.6"}
        ):
