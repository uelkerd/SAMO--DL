...
class TestSecurityIntegration(unittest.TestCase):
    ...
    def test_comprehensive_security_headers(self):
        ...
        for header, validation in required_headers:
            ...
            self.assertIsInstance(
                response.headers[header], str, f"Header {header} should be string"
            )
            if validation == "non-empty":
                self.assertGreater(
                    len(response.headers[header]),
                    0,
    ...
    def test_csp_policy_frame_ancestors(self):
        ...
        csp_policy = self.middleware._build_csp_policy()
        self.assertIn(
            "frame-ancestors 'none'", csp_policy, "Missing frame-ancestors directive"
        )
    ...
    def test_csp_disallows_unsafe_inline_and_eval(self):
        ...
        csp_policy = self.middleware._build_csp_policy()
        self.assertNotIn(
            "'unsafe-inline'", csp_policy, "CSP should not allow unsafe-inline"
        )
        self.assertNotIn(
            "'unsafe-eval'", csp_policy, "CSP should not allow unsafe-eval"
        )
    ...
    def test_permissions_policy_microphone(self):
        ...
        permissions_policy = self.middleware._build_permissions_policy()
        self.assertIn(
            "microphone=()", permissions_policy, "Missing microphone restriction"
        )
    ...
    def test_permissions_policy_geolocation(self):
        ...
        permissions_policy = self.middleware._build_permissions_policy()
        self.assertIn(
            "geolocation=()", permissions_policy, "Missing geolocation restriction"
        )
    ...
    def test_permissions_policy_fullscreen(self):
        ...
        permissions_policy = self.middleware._build_permissions_policy()
        self.assertIn(
            "fullscreen=()", permissions_policy, "Missing fullscreen restriction"
        )
    ...
    def test_production_security_headers(self):
        ...
        self.assertIn(
            "X-Frame-Options", response.headers, "Missing X-Frame-Options header"
        )
        self.assertEqual(
        ...
        self.assertIn(
            "X-XSS-Protection", response.headers, "Missing X-XSS-Protection header"
        )
        self.assertEqual(
        ...
        self.assertIn(
            "Referrer-Policy", response.headers, "Missing Referrer-Policy header"
        )
        self.assertEqual(
