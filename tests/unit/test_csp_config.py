...
class TestCSPConfiguration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = SecurityHeadersConfig(enable_csp=True, enable_content_security_policy=True)
    ...
    def test_csp_fallback_on_missing_csp_key(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump({"security_headers": {"headers": {"X-Frame-Options": "DENY"}}}, f)
            config_path = f.name
    ...
    def test_csp_disabled_when_config_disabled(self):
        ...
        config = SecurityHeadersConfig(enable_csp=False, enable_content_security_policy=False)
    ...
    def test_enhanced_csp_policy_directives(self):
        ...
        for directive, description in required_directives:
            self.assertIn(
                directive, csp_policy, f"Missing CSP directive: {description} ({directive})"
            )
    ...
    def test_csp_policy_production_ready(self):
        ...
        for directive, description in production_security:
            self.assertIn(
                directive, csp_policy, f"Production security missing: {description} ({directive})"
            )
