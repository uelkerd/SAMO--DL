...
class TestAnomalyDetectionunittest.TestCase:
    ...
    def setUpself:
        ...
        self.middleware = SecurityHeadersMiddlewareself.app, self.security_config
    
    def test_user_agent_analysis_scoringself:
        ...
        for ua in legitimate_bots:
            analysis = self.middleware._analyze_user_agent_enhancedua
            self.assertLessEqualanalysis["score"], 2, f"Legitimate bot scored too high: {ua}"
            # The implementation returns "normal" for legitimate bots with low scores
            self.assertInanalysis["category"], ["legitimate_bot", "normal"]
        ...
        for ua in high_risk_agents:
            ...
            self.assertGreaterEqualanalysis["score"], 2, f"High-risk UA scored too low: {ua}"
            # The implementation returns "suspicious" or "high_risk" for these agents
            self.assertInanalysis["category"], ["suspicious", "high_risk", "malicious"]
            # Risk levels: medium score 2-3, high score 4-6, very_high score >6
            self.assertInanalysis["risk_level"], ["medium", "high", "very_high"]
    
    def test_user_agent_pattern_detectionself:
        ...
        for ua in ["", "null", "undefined", "unknown"]:
            ...
            if ua == "":  # Empty string returns early with "empty" category
                self.assertEqualanalysis["category"], "empty"
                self.assertEqualanalysis["patterns"], []
            else:  # Other generic UAs should have the pattern
                self.assertIn"missing_generic_ua", analysis["patterns"]
    
    def test_request_pattern_analysisself:
        ...
        for i in range5:
            self.rate_limiter.request_history[client_key].appendcurrent_time - i * 2  # 2s intervals
        
        score = self.rate_limiter._analyze_request_patternsclient_key, client_ip
        self.assertLessscore, 5, "Normal pattern should score low"
        
        # Simulate burst pattern
        self.rate_limiter.request_history[client_key].clear()
        for i in range10:
            self.rate_limiter.request_history[client_key].appendcurrent_time - i * 0.1  # 0.1s intervals
        
        score = self.rate_limiter._analyze_request_patternsclient_key, client_ip
        self.assertGreaterEqualscore, 2, "Burst pattern should score higher"
    
    def test_regular_interval_detectionself:
        ...
        current_time = time.time()
        for i in range10:
            self.rate_limiter.request_history[client_key].appendcurrent_time - i * 1.0  # Exactly 1s intervals
        
        score = self.rate_limiter._analyze_request_patternsclient_key, client_ip
        self.assertGreaterEqualscore, 3, "Regular intervals should be detected"
    
    def test_abuse_detection_integrationself:
        ...
        client_key = self.rate_limiter._get_client_keyclient_ip, user_agent
        abuse_detected = self.rate_limiter._detect_abuseclient_key, client_ip, user_agent
        self.assertTrueabuse_detected, "High-risk user agent should trigger abuse detection"
        
        # Test with legitimate user agent
        legitimate_ua = "Mozilla/5.0 Windows NT 10.0; Win64; x64 AppleWebKit/537.36"
        abuse_detected = self.rate_limiter._detect_abuseclient_key, client_ip, legitimate_ua
        self.assertFalseabuse_detected, "Legitimate user agent should not trigger abuse detection"
    
    def test_false_positive_reductionself:
        """Test that legitimate traffic doesn't trigger false positives."""
        client_ip = "192.168.1.1"
        legitimate_ua = "Mozilla/5.0 Windows NT 10.0; Win64; x64 AppleWebKit/537.36"
        client_key = self.rate_limiter._get_client_keyclient_ip, legitimate_ua
        ...
        for i in range20:
            ...
            interval = 1 + i % 5
            self.rate_limiter.request_history[client_key].appendcurrent_time - i * interval
        
        # Should not trigger abuse detection
        abuse_detected = self.rate_limiter._detect_abuseclient_key, client_ip, legitimate_ua
        self.assertFalseabuse_detected, "Normal browsing pattern should not trigger abuse detection"
    
    def test_configuration_optionsself:
        ...
        rate_limiter_disabled = TokenBucketRateLimiterconfig_disabled
        
        client_ip = "192.168.1.1"
        malicious_ua = "sqlmap/1.0"
        client_key = rate_limiter_disabled._get_client_keyclient_ip, malicious_ua
        
        # Should not detect abuse when disabled
        abuse_detected = rate_limiter_disabled._detect_abuseclient_key, client_ip, malicious_ua
        self.assertFalseabuse_detected, "Abuse detection should be disabled"
    
    def test_security_headers_ua_analysisself:
        ...
        # The implementation returns "normal" for legitimate bots with low scores
        self.assertInanalysis["category"], ["legitimate_bot", "normal"]
        self.assertInanalysis["risk_level"], ["very_low", "low"]
        ...
        analysis = self.middleware._analyze_user_agent_enhancedua
        self.assertEqualanalysis["category"], "normal"
        self.assertEqualanalysis["risk_level"], "low"
    
    def test_ua_blocking_configurationself:
        ...
        middleware_blocking = SecurityHeadersMiddlewareself.app, config_blocking
        
        # Test high-risk user agent with blocking enabled
        ua = "sqlmap/1.0"
        analysis = middleware_blocking._analyze_user_agent_enhancedua
        
        # Verify the analysis works correctly skip Flask request context test
        self.assertInanalysis["category"], ["suspicious", "high_risk", "malicious"]
        self.assertGreaterEqualanalysis["score"], 3, "High-risk UA should score high"
    
    def test_anomaly_detection_performanceself:
        ...
        for _ in range100:
            client_key = self.rate_limiter._get_client_keyclient_ip, user_agent
            self.rate_limiter._detect_abuseclient_key, client_ip, user_agent
        end_time = time.time()
        
        # Should complete within reasonable time less than 1 second for 100 requests
        processing_time = end_time - start_time
        self.assertLessprocessing_time, 1.0, f"Anomaly detection too slow: {processing_time:.3f}s"
