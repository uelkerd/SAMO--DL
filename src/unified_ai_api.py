...
# ------------------------------
def normalize_emotion_resultsraw: Any -> dict:
    ...
    """
    try:
        if isinstanceraw, dict:
            def _as_floatv: Any -> float:
                try:
                    return floatv
                except Exception:
                    return 1.0
            def _as_strv: Any, default: str = "neutral" -> str:
                try:
                    return strv
                except Exception:
                    return default
            emotions_dict = raw.get"emotions"
            if not isinstanceemotions_dict, dict:
                emotions_dict = {"neutral": 1.0}
            else:
                emotions_dict = {strk: _as_floatv for k, v in emotions_dict.items()}
            return {
        ...
        emotions_attr = getattrraw, "emotions", {"neutral": 1.0}
        emotions = (emotions_attr if isinstanceemotions_attr, dict
                    else {"neutral": 1.0})
        return {
            "emotions": emotions,
            "primary_emotion": str(getattrraw, "primary_emotion", "neutral"),
            "confidence": float(getattrraw, "confidence", 1.0),
            "emotional_intensity": str(
                getattrraw, "emotional_intensity", "neutral"
            ),
        }
    except Exception:
...
def _run_emotion_predicttext: str, threshold: float = 0.5 -> dict:
    ...
    try:
        ...
        if hasattremotion_detector, "predict":
            return emotion_detector.predicttext, threshold=threshold or {}
        # Adapter for BERTEmotionClassifier.predict_emotions
        if hasattremotion_detector, "predict_emotions":
            ...
            result = emotion_detector.predict_emotionstext, threshold=threshold or {}
            probs_list = result.get"probabilities" or []
            if not probs_list:
                return {}
            probs = probs_list[0]
            # Build label->prob mapping
            emotions_map = {label: floatprob for label, prob in zip_LABELS, probs}
            # Determine primary emotion
            if emotions_map:
                primary_label = max(emotions_map.items(), key=lambda kv: kv[1])[0]
                confidence = floatemotions_map[primary_label]
            else:
...
# ------------------------------
def _has_injected_permissionrequest: Request, permission: str -> bool:
...
class WebSocketConnectionManager:
    ...
    def __init__self:
        self.active_connections: Dict[str, Set[WebSocket]] = defaultdictset
        self.connection_metadata: Dict[WebSocket, Dict[str, Any]] = {}
        self.heartbeat_interval = 30  # seconds
        self.max_connections_per_user = 5
        self.connection_timeout = 300  # 5 minutes

    async def connectself, websocket: WebSocket, user_id: str, token: str:
        """Connect a new WebSocket with enhanced management."""
        # Check connection limits
        if lenself.active_connections[user_id] >= self.max_connections_per_user:
            await websocket.closecode=4008, reason="Maximum connections reached"
            return False

        await websocket.accept()
        self.active_connections[user_id].addwebsocket
        ...
        logger.info(
            ...
            "Total connections: %s",
            user_id, lenself.active_connections[user_id]
        )
        return True

    async def disconnectself, websocket: WebSocket:
        ...
        if user_id and websocket in self.active_connections[user_id]:
            self.active_connections[user_id].removewebsocket
    ...
    async def send_personal_message(
        ...
        try:
            await websocket.send_jsonmessage
            if websocket in self.connection_metadata:
    ...
    async def broadcast_to_userself, message: Dict[str, Any], user_id: str:
        ...
        for websocket in self.active_connections[user_id]:
            try:
                ...
                disconnected.addwebsocket

        # Cleanup disconnected connections
        for websocket in disconnected:
            await self.disconnectwebsocket

    async def update_heartbeatself, websocket: WebSocket:
        """Update heartbeat timestamp for connection."""
        if websocket in self.connection_metadata:
            self.connection_metadata[websocket]["last_heartbeat"] = time.time()

    async def cleanup_stale_connectionsself:
        ...
        for websocket, metadata in self.connection_metadata.items():
            if current_time - metadata["last_heartbeat"] > self.connection_timeout:
                stale_connections.appendwebsocket
    ...
    def get_connection_statsself -> Dict[str, Any]:
        """Get connection statistics."""
        total_connections = sum(
            lenconnections for connections in self.active_connections.values()
        )
        total_users = lenself.active_connections

        return {
            "total_connections": total_connections,
            "total_users": total_users,
            "connections_per_user": {
                user_id: lenconnections
                for user_id, connections in self.active_connections.items()
            },
            "connection_metadata": {
                strws: metadata for ws, metadata in self.connection_metadata.items()
            }
        }
...
async def get_current_user(
    credentials: HTTPAuthorizationCredentials = Dependssecurity
) -> TokenPayload:
    """Get current authenticated user from JWT token."""
    token = credentials.credentials
    if payload := jwt_manager.verify_tokentoken:
        return payload
...
def require_permissionpermission: str:
    """Require specific permission for endpoint access."""
    async def permission_checker(
        request: Request,
        current_user: TokenPayload = Dependsget_current_user
    ):
        # Allow tests to inject permissions via header only during pytest runs and
        # explicit toggle
        if _has_injected_permissionrequest, permission:
            return current_user
...
@asynccontextmanager
async def lifespan_: FastAPI -> AsyncGenerator[None, None]:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    ...
                    boollocal_dir, boolarchive_url, boolendpoint_url
                )
...
@app.middleware"http"
async def metrics_middlewarerequest: Request, call_next:
    ...
    resp_status = "500"
    try:
        response = await call_nextrequest
        resp_status = strresponse.status_code
        return response
    finally:
        duration = time.time() - start
        REQUEST_LATENCY.labelsendpoint=endpoint, method=method.observeduration
        REQUEST_COUNT.labelsendpoint=endpoint, method=method, status=resp_status.inc()
...
def _tx_to_dictresult: Any -> Dict[str, Any]:
    """Normalize transcription result dataclass or dict to a plain dict."""
    if isinstanceresult, dict:
        return result
    return {
        "text": getattrresult, "text", "",
        "language": getattrresult, "language", "unknown",
        "confidence": getattrresult, "confidence", 0.0,
        "duration": getattrresult, "duration", 0.0,
        "segments": getattrresult, "segments", [],
        "no_speech_prob": getattrresult, "no_speech_probability", 0.0,
    }
...
async def http_exception_handlerrequest: Request, exc: HTTPException:
    ...
    if exc.status_code in 400, 422:
        return JSONResponsestatus_code=exc.status_code, content={"detail": exc.detail}
    return JSONResponse(
...
def _write_temp_wavcontent: bytes -> str:
    """Persist uploaded audio bytes to a temporary WAV file and return its path."""
    with tempfile.NamedTemporaryFiledelete=False, suffix=".wav" as temp_file:
        temp_file.writecontent
        temp_file.flush()
...
def _infer_quality_from_durationduration: float -> str:
...
def _normalize_transcription_obj(
    ...
    text_val = getattrobj, "text", ""
    lang_val = getattrobj, "language", "unknown"
    conf_val = float(getattrobj, "confidence", 0.0 or 0.0)
    duration = float(getattrobj, "duration", 0.0 or 0.0)
    word_count = getattrobj, "word_count", None
    if word_count is None:
        word_count = len(text_val or "".split())
    speaking_rate = getattrobj, "speaking_rate", None
    if speaking_rate is None:
        speaking_rate = word_count / duration * 60 if duration > 0 else 0.0
    audio_quality = getattrobj, "audio_quality", None
    if audio_quality is None:
        audio_quality = _infer_quality_from_durationduration
    return (
        text_val,
        lang_val,
        conf_val,
...
def _normalize_transcription_attrs(
    ...
    if isinstanceresult, dict:
        return _normalize_transcription_dictresult
    return _normalize_transcription_objresult
...
def _get_request_scoped_summarizermodel: str:
    """Return summarizer for requested model.

    If the requested model differs, attempt to create a request-scoped instance.
    On failure, raise HTTPException400/503 instead of silently falling back.
    """
    if hasattrtext_summarizer, "model_name" and text_summarizer.model_name != model:
        try:
            from src.models.summarization.t5_summarizer import (
                create_t5_summarizer as _create,
            )
            logger.info(
                ...
                model,
                getattrtext_summarizer, "model_name", "unknown",
            )
            return _createmodel
...
# Request Models
class JournalEntryRequestBaseModel:
    ...
    generate_summary: bool = FieldTrue, description="Whether to generate a summary"
...
class EmotionAnalysisBaseModel:
...
class VoiceTranscriptionBaseModel:
...
class CompleteJournalAnalysisBaseModel:
...
async def register_useruser_data: UserRegister -> TokenResponse:
    ...
    try:
        ...
        # Generate tokens
        token_response: TokenResponse = jwt_manager.create_token_pairtoken_user_data
...
async def login_userlogin_data: UserLogin -> TokenResponse:
    ...
    try:
        ...
        # Create user data for token
        user_id = f"user_{hashlogin_data.username % 10000}"
        ...
        permissions = listbase_permissions
        if is_admin_user:
        ...
        token_user_data = {
            ...
            "email": (
                login_data.username if "@" in login_data.username
...
class RefreshTokenRequestBaseModel:
...
async def refresh_tokenrequest: RefreshTokenRequest -> TokenResponse:
    """Refresh access token using refresh token."""
    try:
        # Verify refresh token
        payload = jwt_manager.verify_tokenrequest.refresh_token
        ...
        token_response: TokenResponse = jwt_manager.create_token_pairuser_data
...
async def logout_user(
    ...
    try:
        ...
        if auth_header and auth_header.startswith"Bearer ":
            ...
            # Blacklist the token
            jwt_manager.blacklist_tokentoken
            logger.info(
...
async def get_user_profile(
    ...
    return UserProfile(
        ...
        created_at=datetime.nowtz=timezone.utc.isoformat()
    )


# Simple Chat Contracts minimal
class ChatMessageBaseModel:
    """Single chat message from the user."""
    text: str = Field..., min_length=1, description="User message text"
    summarize: bool = FieldFalse, description="Summarize response using T5"
    model: str = Field"t5-small", description="Summarizer model if summarize=true"


class ChatResponseBaseModel:
    """Chat response payload."""
    reply: str
    summary: Optional[str] = None
    meta: Dict[str, Any] = Fielddefault_factory=dict
...
async def chat_http(
    ...
    if message.summarize:
        ...
        summarizer_instance = _get_request_scoped_summarizermessage.model
...
async def chat_websocket(websocket: WebSocket, token: str = QueryNone) -> None:
    ...
    if not auth_token:
        try:
            ...
            auth_token = json.loadsinitial.get"token"
            ...
            await websocket.closecode=4001
            return
    ...
    try:
        payload = jwt_manager.verify_tokenauth_token
    except Exception:
        ...
        while True:
            ...
            if summarize_flag and text:
                try:
                    if text_summarizer is None:
                        _ensure_summarizer_loaded()
                    summarizer_instance = _get_request_scoped_summarizermodel
                    summary_text = summarizer_instance.generate_summary(
                        reply, max_length=80, min_length=20
                    )
                    ...
                    response["summary_error"] = strexc

            await websocket.send_jsonresponse
...
async def analyze_journal_entry(
    ...
    try:
        ...
        if not request.text.strip():
            raise HTTPExceptionstatus_code=400, detail="Text cannot be empty"
        ...
        if emotion_detector is not None:
            try:
                ...
                emotion_results = normalize_emotion_resultsraw
                logger.info(
        ...
        if text_summarizer is not None and request.generate_summary:
            try:
                summary_results = text_summarizer.summarizerequest.text
                logger.info"✅ Text summarization completed"
            except Exception as exc:
                logger.warning"⚠️  Text summarization failed: %s", exc
                summary_results = {
                    "summary": (
                        request.text[:200] + "..." if lenrequest.text > 200
                        else request.text
                    ),
                    "key_emotions": (
                        [emotion_results["primary_emotion"]] if emotion_results
                        else ["neutral"]
                    ),
        ...
        raise HTTPExceptionstatus_code=500, detail="Analysis failed" from exc
...
async def analyze_voice_journal(
    ...
    generate_summary: bool = FormTrue, description="Whether to generate a summary",
    ...
    try:
        ...
        if voice_transcriber is not None:
            try:
                ...
                try:
                    ...
                    Pathtemp_file_path.unlinkmissing_ok=True
        ...
        text_analysis = await analyze_journal_entrytext_request, x_api_key
        ...
        if transcription_results:
            (
                ...
                _speaking_rate,
                _audio_quality,
            ) = _normalize_transcription_attrstranscription_results
            # Validate required fields before constructing VoiceTranscription
            if not isinstance_text, str or _text is None:
                logger.warning(
                ...
                normalized_tx = {
                    ...
                    "confidence": float_conf if _conf is not None else 0.0,
                    "duration": float_duration if _duration is not None else 0.0,
                    "word_count": int_word_count if _word_count is not None else 0,
                    "speaking_rate": (
                        float_speaking_rate if _speaking_rate is not None else 0.0
                    ),
        ...
        raise HTTPExceptionstatus_code=500, detail="Voice analysis failed" from exc
...
async def transcribe_voice(
    ...
    timestamp: bool = FormFalse, description="Include word-level timestamps",
    current_user: TokenPayload = Dependsget_current_user,
) -> VoiceTranscription:
    ...
    try:
        ...
        if not audio_file.filename:
            raise HTTPExceptionstatus_code=400, detail="Audio file required"
        ...
        await audio_file.seek0

        # Save uploaded file temporarily
        temp_file_path = _write_temp_wavcontent
        ...
        try:
            ...
            sig = inspect.signaturevoice_transcriber.transcribe
                ...
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            ...
                            "calls: %s; %s",
                            repre_positional, repre_fallback
                        )
                        raise
            else:
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            logger.error(
                                ...
                                repre_kwargs, repre_positional, repre_fallback
                            )
                            raise
            ...
            (
                ...
                speaking_rate,
                audio_quality,
            ) = _normalize_transcription_attrstranscription_result
        ...
        if isinstanceexc, HTTPException:
...
async def batch_transcribe_voice(
    ...
    try:
        ...
        if (not _has_injected_permissionrequest, "batch_processing" and
        ...
        for i, audio_file in enumerateaudio_files:
            try:
                ...
                if audio_file.filename:
                    prefix = f"{Pathaudio_file.filename.stem}_"
                else:
                    prefix = "file_"
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=".wav", prefix=prefix
                ) as temp_file:
                    temp_file.writecontent or b""
                    temp_file.flush()  # Ensure data is written to disk
                    temp_file_path = temp_file.name
...
async def summarize_text(
    ...
    model: str = Form(
        "t5-small",
        description="Summarization model t5-small, t5-base, t5-large"
    ),
    max_length: int = Form150, description="Maximum summary length", ge=10, le=500,
    min_length: int = Form30, description="Minimum summary length", ge=5, le=200,
    ...
    try:
        ...
        for call in (
            ...
            lambda: summarizer_instance.generate_summarytext, max_length, min_length,
            lambda: summarizer_instance.generate_summarytext,
        ):
            try:
                summary_text = call()
                break
        ...
        if summary_text is None:
            ...
            raise HTTPException(
                status_code=500, detail="Text summarization failed"
            )
        ...
        emotional_tone, key_emotions = _derive_emotionsummary_text or ""
...
async def websocket_realtime_processing(websocket: WebSocket, token: str = QueryNone):
    """WebSocket endpoint for real-time voice processing."""
    # Validate authentication token
    if not token:
        await websocket.closecode=4001, reason="Authentication token required"
        return

    try:
        # Verify JWT token using the global jwt_manager instance
        payload = jwt_manager.verify_tokentoken
        if not payload:
            await websocket.closecode=4001, reason="Invalid authentication token"
            return

        # Check if user has real-time processing permission
        if "realtime_processing" not in payload.permissions:
            await websocket.closecode=4003, reason="Insufficient permissions"
            return

    except Exception as e:
        await websocket.close(code=4001, reason=f"Authentication failed: {stre}")
        return
    ...
    try:
        ...
        if not token:
            ...
            try:
                message_data = json.loadsinitial_message
        ...
        while True:
            ...
            if voice_transcriber:
                try:
                    ...
                    try:
                        # Transcribe
                        result = voice_transcriber.transcribetemp_file_path
...
async def get_performance_metrics(
    ...
    try:
        ...
        cpu_percent = await asyncio.to_threadpsutil.cpu_percent, interval=1
        memory = await asyncio.to_threadpsutil.virtual_memory
        disk = await asyncio.to_threadpsutil.disk_usage, '/'
...
async def detailed_health_check(
    ...
    if emotion_detector is None:
        ...
        try:
            ...
            health_status = "degraded"
            issues.appendf"Emotion detection model error: {exc}"
            model_checks["emotion_detection"] = {"status": "error", "error": strexc}
    ...
    if text_summarizer is None:
        ...
        try:
            ...
            health_status = "degraded"
            issues.appendf"Text summarization model error: {exc}"
            model_checks["text_summarization"] = {"status": "error", "error": strexc}
    ...
    try:
        ...
        if cpu_percent > 90:
            health_status = "degraded"
            issues.appendf"High CPU usage: {cpu_percent}%"

        if memory.percent > 90:
            health_status = "degraded"
            issues.appendf"High memory usage: {memory.percent}%"
        ...
        system_checks = {"status": "error", "error": strexc}
        health_status = "degraded"
        issues.appendf"System check failed: {exc}"
...
if __name__ == "__main__":
    uvicorn.runapp, host="0.0.0.0", port=8000
