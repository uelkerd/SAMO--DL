...
class CompleteJournalAnalysis(BaseModel):
    ...
    transcription: Optional[VoiceTranscription] = Field(None, description="Voice transcription results")
    ...
    try:
        ...
        if text_summarizer and generate_summary:
            try:
                ...
                key_emotions = [
                    emotion for emotion, score in emotion_analysis.emotions.items() 
...
async def get_models_status():
    ...
    return {
        ...
        "text_summarization": {
            ...
            "model_type": "T5/BART",
            "capabilities": ["emotional context preservation", "adaptive length", "batch processing"]
        },
        "voice_processing": {
            "loaded": voice_transcriber is not None,
            "model_type": "OpenAI Whisper",
            "capabilities": ["multi-format audio", "language detection", "quality assessment"]
        },
