...
class TokenBucketRateLimiter:
    ...
    def _calculate_request_regular_interval_score(recent_history: list) -> int:
        ...
        if len(recent_history) < 5:
            return 0
        intervals = [
            recent_history[i] - recent_history[i - 1] for i in range(1, len(recent_history))
        ]
        if len(intervals) < 3:
            return 0
    ...
    @staticmethod
    def _calculate_sustained_volume_score(recent_history: list, current_time: float) -> int:
