...
class DataValidator:
    ...
    def check_data_types(
        ...
        for column, expected_type in expected_types.items():
            ...
            if (expected_type in (int, float) and 
                pd.api.types.is_numeric_dtype(actual_type)):
                type_check_results[column] = True
            # Handle string types
            elif expected_type is str and pd.api.types.is_string_dtype(actual_type):
                type_check_results[column] = True
            # Handle datetime types
            elif (expected_type is pd.Timestamp and 
                  pd.api.types.is_datetime64_any_dtype(actual_type)):
                type_check_results[column] = True
            # Handle boolean types
                ...
                if not is_match:
                    logger.warning(
                        "Column '{column}' has type {actual_type}, "
    ...
    def check_text_quality(self, df: pd.DataFrame, 
        ...
        result_df["word_count"] = (result_df[text_column].astype(str)
        ...
        if empty_count > 0:
            logger.warning("Found {empty_count} empty entries in "
                          "'{text_column}' column")

        if very_short_count > 0:
            logger.warning(
                "Found {very_short_count} very short entries (< 5 words) in "
                "'{text_column}' column"
            )
    ...
    def validate_journal_entries(
        ...
        has_missing_required = any(missing_stats.get(col, 0) > 0 
...
def validate_text_input(input_text: str, min_length: int = 1, 
                       max_length: int = 10000) -> Dict[str, Union[bool, str]]:
    ...
    if len(stripped_text) < min_length:
        return {"is_valid": False, 
                "error": f"Text is too short, must be at least "
    ...
    for pattern in harmful_patterns:
        if pattern.lower() in input_text.lower():
            return {"is_valid": False, 
                    "error": f"Text contains potentially harmful content: "
                             f"{pattern}"}
