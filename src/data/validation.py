...
class DataValidator:
    ...
    def check_missing_values(self,
                             df: pd.DataFrame,
                             required_columns: list[str] | None = None) -> dict[str, float]:
        ...
        for column in df.columns:
            ...
            if column in required_columns and missing_count > 0:
                logger.warning(f"Required column '{column}' has {missing_count} missing values ({missing_percent:.2f}%)")
    ...
    def check_data_types(self,
        ...
        for column, expected_type in expected_types.items():
            ...
            if (expected_type in (int, float) and pd.api.types.is_numeric_dtype(actual_type)) or (expected_type == str and pd.api.types.is_string_dtype(actual_type)):
    ...
    def check_text_quality(self,
        ...
        result_df["is_empty"] = result_df[text_column].astype(str).apply(lambda x: len(x.strip()) == 0)
        ...
        if very_short_count > 0:
            logger.warning(f"Found {very_short_count} very short entries (< 5 words) in '{text_column}' column")
    ...
    def validate_journal_entries(self,
                                ...
                                expected_types: dict[str, type] | None = None) -> tuple[bool, pd.DataFrame]:
