...
from .dataset_loader import GOEMOTIONS_EMOTIONS, EMOTION_ID_TO_LABEL
...
class BERTEmotionClassifier(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"Initialized BERT emotion classifier with {self.count_parameters():,} parameters")
    ...
    def unfreeze_bert_layers(self, num_layers: int):
        ...
        currently_frozen = sum(
            1 for layer in self.bert.encoder.layer
...
class WeightedBCELoss(nn.Module):
    ...
    def __init__(self, class_weights: Optional[torch.Tensor] = None, reduction: str = 'mean'):
        ...
        if class_weights is not None:
            logger.info(f"Initialized WeightedBCELoss with class weights: min={class_weights.min():.3f}, max={class_weights.max():.3f}")
...
def create_bert_emotion_classifier(
    ...
    logger.info(f"Created BERT emotion classifier: {model.count_parameters():,} trainable parameters")
...
def evaluate_emotion_classifier(
    ...
    logger.info(f"Evaluation complete - Micro F1: {metrics['micro_f1']:.3f}, Macro F1: {metrics['macro_f1']:.3f}")
...
if __name__ == "__main__":
    ...
    print(f"\nTest text: {test_text}")
    print(f"Primary emotion: {predictions['primary_emotion']} (confidence: {predictions['primary_confidence']:.3f})")
