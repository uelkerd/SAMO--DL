    ...
    def predict(self, text: str, threshold: float = 0.5) -> Dict:
        ...
        try:
            ...
            if emotions:
                ...
                primary_label, confidence = "neutral", 1.0
            intensity = "high" if confidence >= 0.75 else (
                                                           "moderate" if confidence >= 0.4 else "low"
                                                          )
            return {
                "emotions": emotions,
                "primary_emotion": primary_label,
...
def _wrap_local_model(
                      ...
                      force_multi_label: Optional[bool] = None) -> HFEmotionDetector:
    ...
    id2label = getattr(
                       cfg,
                       "id2label",
                       None) or {i: str(i) for i in range(cfg.num_labels)}
...
def load_emotion_model_multi_source(
    ...
    """Try multiple sources to load the emotion model. Returns an object with .predict(
                                                                                       text,
                                                                                       threshold).
    ...
    if archive_url:
        try:
            ...
            archive_path = os.path.join(
                                        cache_dir,
                                        os.path.basename(archive_url.split("?")[0])
                                       )
            ...
            candidates = [extract_dir] + [os.path.join(
                                                       extract_dir,
                                                       d) for d in os.listdir(extract_dir)]
            for cand in candidates:
                if os.path.isdir(
                                 cand) and os.path.exists(os.path.join(cand,
                                 "config.json")):
                    try:
