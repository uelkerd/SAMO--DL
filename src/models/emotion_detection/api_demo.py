...
class EmotionRequest(BaseModel):
    ...
    top_k: Optional[int] = Field(5, description="Number of top emotions to return", ge=1,
        le=28)
...
class EmotionResponse(BaseModel):
    ...
    primary_emotion: str = Field(..., description="Emotion with highest confidence",
        example="joy")
    confidence: float = Field(
        ..., description="Confidence score for primary emotion", ge=0.0, le=1.
            0, example=0.
            85
    )
    ...
    all_probabilities: List[float] = Field(
        ..., description="Probabilities for all emotions", example=[0.85, 0.72, 0.64, 0.
            0, 0.
            0]
    )
