...
def check_training_logs():
    ...
    if found_logs:
        logger.info(f"📁 Found {len(found_logs)} log files:")
...
def test_bce_loss():
    ...
    loss = F.binary_cross_entropy_with_logits(logits, labels)
    print(f"All zeros - Loss: {loss.item():.6f}")
    ...
    loss = F.binary_cross_entropy_with_logits(logits, labels)
    print(f"All ones - Loss: {loss.item():.6f}")
    ...
    loss = F.binary_cross_entropy_with_logits(logits, labels)
    print(f"Perfect predictions - Loss: {loss.item():.6f}")
    ...
    loss = F.binary_cross_entropy_with_logits(logits, labels)
    print(f"Small logits - Loss: {loss.item():.6f}")
