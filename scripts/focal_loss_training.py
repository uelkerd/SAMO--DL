#!/usr/bin/env python3
"""
Focal Loss Training for Emotion Detection

This script implements Focal Loss to address class imbalance and improve F1 scores.
Focal Loss focuses training on hard examples and reduces the impact of easy examples.

Usage:
    python scripts/focal_loss_training.py [--gamma 2.0] [--alpha 0.25]
"""
...
def train_with_focal_loss(
    ...
    datasets = data_loader.prepare_data(dev_mode=False)  # Use full dataset
            ...
            torch.save(
                {
                    ...
                    "focal_loss_params": {"gamma": gamma, "alpha": alpha},
...
def main():
    ...
    try:
        ...
        logger.info(f"   • Best Loss: {results['best_val_loss']:.4f}")
        logger.info("   • Next: Run threshold optimization")
