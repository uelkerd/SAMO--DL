...
class DemoSecurityAuditor:
                        ...
                        for pattern in input_patterns:
                            if re.search(pattern, line, re.IGNORECASE):
                                ...
                                sanitization_found = any(
                                    "sanitize" in lines[j].lower()
                                    or "escape" in lines[j].lower()
                                    or "validate" in lines[j].lower()
    ...
    def check_dependency_security(self):
        ...
        if package_json.exists():
            try:
                ...
                vulnerable_packages = ["jquery@1.x", "lodash@4.17.0", "moment@2.19.0"]

                dependencies = package_data.get("dependencies", {})
                for package_name, version in dependencies.items():
                    if any(
                        vuln_pkg.split("@")[0] in package_name for vuln_pkg in vulnerable_packages
                    ):
                        warning = {
    ...
    def check_file_permissions(self):
        ...
        sensitive_files = ["config.js", "api-key.js", "secrets.js", ".env"]
    ...
    def calculate_security_score(self):
        ...
        _total_issues = len(self.audit_results["vulnerabilities"]) + len(
            self.audit_results["warnings"]
        )
        high_severity = len(
            [v for v in self.audit_results["vulnerabilities"] if v["severity"] == "HIGH"]
        )
        medium_severity = len(
            [v for v in self.audit_results["vulnerabilities"] if v["severity"] == "MEDIUM"]
        )
    ...
    def generate_security_report(self):
        ...
        print(
            f"🚨 Vulnerabilities: {summary['total_vulnerabilities']} (High: {summary['high_severity']}, Medium: {summary['medium_severity']})"
        )
        ...
        if self.audit_results["warnings"]:
            ...
            for warning in self.audit_results["warnings"]:
                print(
                    f"  [{warning['severity']}] {warning['type']} in {warning['file']}:{warning['line']}"
                )
    ...
    def save_security_report(self):
        ...
        report_file = (
            ...
            / "artifacts"
            / "security-reports"
            / f"demo_security_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser.add_argument(
        "--output-format", "-f", choices=["json", "html"], default="json", help="Output format"
    )

    args = parser.parse_args()

    # Create security auditor
    auditor = DemoSecurityAuditor(verbose=args.verbose, output_format=args.output_format)
