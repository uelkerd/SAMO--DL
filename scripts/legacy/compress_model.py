...
def compress_modelinput_model: str, output_model: str -> bool:
    ...
    try:
        ...
        original_inference_time = benchmark_inferencemodel
        ...
        torch.quantization.preparemodel, inplace=True

        quantized_model = torch.quantization.quantize_dynamic(
            model, {torch.nn.Linear}, dtype=torch.qint8
        )

        quantized_size = get_model_sizequantized_model
        ...
        quantized_inference_time = benchmark_inferencequantized_model
        ...
        if isinstancecheckpoint, dict and "model_state_dict" in checkpoint:
            ...
            torch.savecheckpoint, output_path
        else:
...
def get_model_sizemodel: torch.nn.Module -> float:
...
def benchmark_inferencemodel: torch.nn.Module, num_runs: int = 50 -> float:
    ...
    dummy_input = {
        ...
        "attention_mask": torch.ones1, 128,
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParserdescription="Compress BERT emotion classifier model"
    parser.add_argument(
        "--input_model",
        type=str,
    ...
    success = compress_modelargs.input_model, args.output_model
