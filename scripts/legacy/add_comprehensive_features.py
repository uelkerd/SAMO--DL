...
def add_comprehensive_features():
    ...
    advanced_cells = [
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "print(f'Original model labels: {AutoModelForSequenceClassification.from_pretrainedmodel_name.config.num_labels}')\n",
                "print(f'Original id2label: {AutoModelForSequenceClassification.from_pretrainedmodel_name.config.id2label}')\n",
                "\n",
                ...
                "    model_name,\n",
                "    num_labels=lenemotions,  # Set to 12 emotions\n",
                ...
                "printf'✅ Model created with {model.config.num_labels} labels'\n",
                "printf'✅ New id2label: {model.config.id2label}'\n",
                "printf'✅ Classifier output size: {model.classifier.out_proj.out_features}'\n",
                ...
                "    printf'✅ Test output shape: {test_output.logits.shape}'\n",
                "    print(f'✅ Expected shape: [1, {lenemotions}]')\n",
                "    assert test_output.logits.shape[1] == lenemotions, f'Output shape mismatch: {test_output.logits.shape[1]} != {lenemotions}'\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "    'balanced',\n",
                "    classes=np.uniquetrain_labels,\n",
                "    y=train_labels\n",
                ")\n",
                "\n",
                "class_weights_tensor = torch.FloatTensorclass_weights\n",
                "if torch.cuda.is_available():\n",
                "    class_weights_tensor = class_weights_tensor.cuda()\n",
                "\n",
                "printf'✅ Class weights calculated: {class_weights}'\n",
                "printf'✅ Class weights tensor shape: {class_weights_tensor.shape}'\n",
                "\n",
                "# Focal Loss implementation\n",
                "class FocalLosstorch.nn.Module:\n",
                "    def __init__self, alpha=1, gamma=2:\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "    def __init__self, focal_alpha=1, focal_gamma=2, class_weights=None, *args, **kwargs:\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "# Apply preprocessing\n",
                "train_dataset_processed = preprocess_functiontrain_dataset\n",
                "val_dataset_processed = preprocess_functionval_dataset\n",
                "\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "print(f'📈 Data augmentation: {lenaugmented_data} samples added')\n",
                "\n",
                "# Train the model\n",
                "trainer.train()\n",
                "\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "print'\\n📋 DETAILED CLASSIFICATION REPORT:'\n",
                "print'=' * 40\n",
                "predictions = trainer.predictval_dataset_processed\n",
                "pred_labels = np.argmaxpredictions.predictions, axis=1\n",
                "true_labels = val_labels\n",
                "\n",
                "print(classification_reporttrue_labels, pred_labels, target_names=emotions)"
            ]
        },
        {
            "cell_type": "markdown",
        ...
        },
        {
            ...
            "source": [
                ...
                "print'=' * 40\n",
                "\n",
                "for i, example in enumerateunseen_examples, 1:\n",
                "    inputs = tokenizerexample, return_tensors='pt', truncation=True, padding=True\n",
                "    if torch.cuda.is_available():\n",
                ...
                "    percentage = (count / lenpred_labels) * 100\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "trainer.save_modelmodel_save_path\n",
                "tokenizer.save_pretrainedmodel_save_path\n",
                "\n",
                "printf'✅ Model saved to: {model_save_path}'\n",
                "\n",
                ...
                "saved_model = AutoModelForSequenceClassification.from_pretrainedmodel_save_path\n",
                "saved_tokenizer = AutoTokenizer.from_pretrainedmodel_save_path\n",
                "\n",
                "printf'✅ Saved model labels: {saved_model.config.num_labels}'\n",
                "printf'✅ Saved id2label: {saved_model.config.id2label}'\n",
                "printf'✅ Saved label2id: {saved_model.config.label2id}'\n",
                "printf'✅ Saved problem_type: {saved_model.config.problem_type}'\n",
                "\n",
                ...
                "    predicted_label = torch.argmaxtest_output.logits, dim=1.item()\n",
                "    confidence = torch.softmaxtest_output.logits, dim=1[0][predicted_label].item()\n",
                "\n",
                "print'\\n🧪 SAVED MODEL TEST:'\n",
                ...
                "config_correct = (\n",
                "    saved_model.config.num_labels == lenemotions and\n",
                "    saved_model.config.id2label == {i: emotion for i, emotion in enumerateemotions} and\n",
    ...
    notebook['cells'].extendadvanced_cells
