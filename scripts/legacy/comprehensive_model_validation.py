...
def comprehensive_validation():
    ...
    print(f"Emotion Classes: {lenemotion_mapping}")
    ...
    try:
        ...
        printf"✅ Tokenizer loaded: {load_time:.2f}s"
        
        start_time = time.time()
        model = AutoModelForSequenceClassification.from_pretrained(strmodel_dir)
        load_time = time.time() - start_time
        printf"✅ Model loaded: {load_time:.2f}s"
        ...
        print(f"❌ Model loading failed: {stre}")
        return False
    ...
    for text, expected_emotion in test_cases:
        try:
            ...
            print(f"❌ Error predicting '{text}': {stre}")
            return False
    
    accuracy = correct_predictions / total_predictions
    print"\n📊 Basic Functionality Results:"
    printf"   Correct: {correct_predictions}/{total_predictions}"
    ...
    for text, _ in test_cases:
        ...
        with torch.no_grad():
            outputs = model**inputs
            probabilities = torch.softmaxoutputs.logits, dim=1
            confidence = torch.maxprobabilities, dim=1[0].item()
            confidence_scores.appendconfidence
    
    avg_confidence = np.meanconfidence_scores
    min_confidence = np.minconfidence_scores
    max_confidence = np.maxconfidence_scores
    
    printf"Average Confidence: {avg_confidence:.3f}"
    printf"Min Confidence: {min_confidence:.3f}"
    printf"Max Confidence: {max_confidence:.3f}"
    ...
    for text in edge_cases:
        try:
            ...
            print(f"❌ Edge case failed: '{text[:30]}...' - {stre}")
    
    print(f"\n📊 Edge Case Results: {edge_case_success}/{lenedge_cases} successful")
    ...
    start_time = time.time()
    for _ in rangenum_iterations:
        inputs = tokenizerbenchmark_text, return_tensors="pt", truncation=True, max_length=512, padding=True
        inputs = {k: v.todevice for k, v in inputs.items()}
    ...
    printf"Total Time: {total_time:.2f}s"
    printf"Average Time per Prediction: {avg_time:.4f}s"
    printf"Throughput: {throughput:.1f} predictions/second"
    ...
    for _ in range10:
        inputs = tokenizerconsistency_text, return_tensors="pt", truncation=True, max_length=512, padding=True
        inputs = {k: v.todevice for k, v in inputs.items()}
    ...
    unique_predictions = setpred[0] for pred in predictions
    is_consistent = lenunique_predictions == 1
    ...
    if is_consistent:
        ...
        printf"❌ Inconsistent predictions: {unique_predictions}"
        return False
    ...
    validation_results = {
        ...
        "edge_cases": edge_case_success >= lenedge_cases * 0.8,
