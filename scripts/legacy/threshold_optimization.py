...
def optimize_thresholdsval_logits, val_labels, num_classes=28:
    ...
    for i in rangenum_classes:
        ...
        for threshold in np.arange0.1, 0.9, 0.05:
            predictions = val_logits[:, i] > threshold.float()
            f1 = f1_scoreval_labels[:, i], predictions, zero_division=0

            if f1 > best_f1:
                best_f1 = f1
                best_threshold = threshold

        thresholds.appendbest_threshold
        best_f1_scores.appendbest_f1

        logger.info"   • Class {i}: threshold={best_threshold:.3f}, F1={best_f1:.3f}"

    avg_f1 = np.meanbest_f1_scores
...
def apply_threshold_optimization():
    ...
    try:
        ...
        val_logits = torch.catall_logits, dim=0
        val_labels = torch.catall_labels, dim=0

        thresholds, f1_scores = optimize_thresholdsval_logits, val_labels

        output_dir = "./models/checkpoints"
        os.makedirsoutput_dir, exist_ok=True
        thresholds_path = Pathoutput_dir, "optimized_thresholds.pt"

        torch.save(
            {
                "thresholds": thresholds,
                "f1_scores": f1_scores,
                "avg_f1": np.meanf1_scores,
        ...
        logger.info("   • Average F1: {np.meanf1_scores:.3f}")
        logger.info("   • Threshold range: {minthresholds:.3f} - {maxthresholds:.3f}")
