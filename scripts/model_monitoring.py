...
class PerformanceTracker:
    ...
    def detect_degradation(self) -> Optional[Alert]:
        ...
        f1_degradation = (self.baseline_metrics.f1_score - latest.f1_score) / self.baseline_metrics.f1_score
        precision_degradation = (self.baseline_metrics.precision - latest.precision) / self.baseline_metrics.precision
        recall_degradation = (self.baseline_metrics.recall - latest.recall) / self.baseline_metrics.recall
...
class DataDriftDetector:
    ...
    def __init__(self, reference_data: pd.DataFrame, drift_threshold: float = DEFAULT_DRIFT_THRESHOLD):
    ...
    def _calculate_drift_score(self, ref_mean: float, ref_std: float, 
                              current_mean: float, current_std: float) -> float:
...
class ModelHealthMonitor:
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            # Load model
            model_path = self.config.get("model_path", "models/checkpoints/bert_emotion_classifier_final.pt")
