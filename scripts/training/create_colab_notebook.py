...
def create_colab_notebook():
        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "\n",
                    "def analyze_writing_styletexts, domain_name:\n",
                    "    \"\"\"Analyze writing style characteristics of a domain.\"\"\"\n",
                    "    avg_length = np.mean([len(text.split()) for text in texts])\n",
                    "    personal_pronouns = sum['I ' in text or 'my ' in text or 'me ' in text for text in texts] / lentexts\n",
                    "    reflection_words = sum(['think' in text.lower() or 'feel' in text.lower() or 'believe' in text.lower() \n",
                    "                           for text in texts]) / lentexts\n",
                    "    \n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "            return focal_loss\n",
                    "\n",
                    "class DomainAdaptedEmotionClassifiernn.Module:\n",
                    "    \"\"\"BERT-based emotion classifier with domain adaptation capabilities.\"\"\"\n",
                    "    \n",
                    "    def __init__self, model_name=\"bert-base-uncased\", num_labels=12, dropout=0.3:\n",
                    "        super().__init__()\n",
                    ...
                    "model = DomainAdaptedEmotionClassifiermodel_name=model_name, num_labels=12\n",
                    "\n",
                    "device = torch.device(\"cuda\" if torch.cuda.is_available() else \"cpu\")\n",
                    "model = model.todevice\n",
                    "\n",
                    "printf\"✅ Model loaded on {device}\"\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "all_emotions = list(setgo_single_labels + journal_emotions)\n",
                    "label_encoder.fitall_emotions\n",
                    "\n",
                    "# Encode labels\n",
                    "go_encoded_labels = label_encoder.transformgo_single_labels\n",
                    "journal_encoded_labels = label_encoder.transformjournal_emotions\n",
                    "\n",
                    "# Split journal data\n",
                    ...
                    "\n",
                    "# Create datasets\n",
                    "go_dataset = EmotionDatasetgo_texts, go_encoded_labels, tokenizer\n",
                    "journal_train_dataset = EmotionDatasetjournal_train_texts, journal_train_labels, tokenizer\n",
                    "journal_val_dataset = EmotionDatasetjournal_val_texts, journal_val_labels, tokenizer\n",
                    "\n",
                    "# Create dataloaders\n",
                    "batch_size = 16\n",
                    "go_loader = DataLoadergo_dataset, batch_size=batch_size, shuffle=True, num_workers=2\n",
                    "journal_train_loader = DataLoaderjournal_train_dataset, batch_size=batch_size, shuffle=True, num_workers=2\n",
                    "journal_val_loader = DataLoaderjournal_val_dataset, batch_size=batch_size, shuffle=False, num_workers=2\n",
                    "\n",
                    "printf\"✅ Data prepared:\"\n",
                    "print(f\"  GoEmotions: {lengo_dataset} samples\")\n",
                    "print(f\"  Journal Train: {lenjournal_train_dataset} samples\")\n",
                    "print(f\"  Journal Val: {lenjournal_val_dataset} samples\")\n",
                    "print(f\"  Total classes: {lenlabel_encoder.classes_}\")"
                ]
            },
            {
                "cell_type": "markdown",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    \n",
                    "    def __init__self, model, tokenizer, device:\n",
                    "        self.model = model\n",
                    "        self.tokenizer = tokenizer\n",
                    "        self.device = device\n",
                    "        self.criterion = FocalLossalpha=1, gamma=2\n",
                    "        self.domain_criterion = nn.CrossEntropyLoss()\n",
                    "        \n",
                    "    def train_stepself, batch, domain_labels, lambda_domain=0.1:\n",
                    ...
                    "        domain_labels = domain_labels.toself.device\n",
                    "        \n",
                    "        # Forward pass\n",
                    "        emotion_logits, domain_logits = self.modelinput_ids, attention_mask, domain_labels\n",
                    "        \n",
                    "        # Calculate losses\n",
                    "        emotion_loss = self.criterionemotion_logits, labels\n",
                    "        domain_loss = self.domain_criteriondomain_logits, domain_labels\n",
                    "        \n",
                    "        # Combined loss\n",
                    ...
                    "                labels = batch['labels'].toself.device\n",
                    "                \n",
                    "                emotion_logits = self.modelinput_ids, attention_mask\n",
                    "                loss = self.criterionemotion_logits, labels\n",
                    "                \n",
                    "                total_loss += loss.item()\n",
                    "                predictions = torch.argmaxemotion_logits, dim=1\n",
                    "                \n",
                    ...
                    "        f1_weighted = f1_scoreall_labels, all_predictions, average='weighted'\n",
                    "        \n",
                    "        return {\n",
                    "            'loss': total_loss / lendataloader,\n",
                    "            'f1_macro': f1_macro,\n",
                    "            'f1_weighted': f1_weighted\n",
                    "        }\n",
                    "\n",
                    "# Initialize trainer\n",
                    "trainer = DomainAdaptationTrainermodel, tokenizer, device\n",
                    "optimizer = torch.optim.AdamW(model.parameters(), lr=2e-5, weight_decay=0.01)\n",
                    "\n",
                    "# Initialize wandb optional\n",
                    "try:\n",
                    "    wandb.initproject=\"samo-domain-adaptation\", name=\"journal-emotion-detection\"\n",
                    "    use_wandb = True\n",
                    "except:\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "        domain_labels = torch.zeros(batch['input_ids'].size0, dtype=torch.long)\n",
                    "        losses = trainer.train_stepbatch, domain_labels, lambda_domain=0.1\n",
                    "        \n",
                    "        optimizer.zero_grad()\n",
                    ...
                    "        \n",
                    "        if i % 100 == 0:\n",
                    "            print(f\"    Batch {i}/{lengo_loader}, Loss: {losses['total_loss'].item():.4f}\")\n",
                    "    \n",
                    "    # Train on journal data\n",
                    "    print\"  📝 Training on journal data...\"\n",
                    "    for i, batch in enumeratejournal_train_loader:\n",
                    "        domain_labels = torch.ones(batch['input_ids'].size0, dtype=torch.long)\n",
                    "        losses = trainer.train_stepbatch, domain_labels, lambda_domain=0.1\n",
                    "        \n",
                    "        optimizer.zero_grad()\n",
                    "        losses['total_loss'].backward()\n",
                    ...
                    "        \n",
                    "        if i % 10 == 0:\n",
                    "            print(f\"    Batch {i}/{lenjournal_train_loader}, Loss: {losses['total_loss'].item():.4f}\")\n",
                    "    \n",
                    "    # Validation\n",
                    "    print\"  🎯 Validating on journal test set...\"\n",
                    "    val_results = trainer.evaluatejournal_val_loader\n",
                    "    \n",
                    "    avg_loss = total_loss / (lengo_loader + lenjournal_train_loader)\n",
                    "    \n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "history_df = pd.DataFrametraining_history\n",
                    "\n",
                    "fig, axes = plt.subplots(1, 2, figsize=15, 5)\n",
                    "\n",
                    "# Loss plot\n",
                    "axes[0].plothistory_df['epoch'], history_df['train_loss'], 'b-', label='Training Loss'\n",
                    ...
                    "\n",
                    "# F1 Score plot\n",
                    "axes[1].plothistory_df['epoch'], history_df['val_f1_macro'], 'r-', label='F1 Macro'\n",
                    "axes[1].plothistory_df['epoch'], history_df['val_f1_weighted'], 'g-', label='F1 Weighted'\n",
                    ...
                    "final_results = trainer.evaluatejournal_val_loader\n",
                    "\n",
                    ...
                    "printf\"\\n🎯 REQ-DL-012 Validation:\"\n",
                    "printf\"  Target: 70% F1 score on journal entries\"\n",
                    "printf\"  Achieved: {final_results['f1_macro']:.1%} F1 score\"\n",
                    "printf\"  Status: {'✅ SUCCESS' if final_results['f1_macro'] >= 0.7 else '❌ NEEDS IMPROVEMENT'}\""
                ]
            },
            {
                "cell_type": "markdown",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "    json.dumpmodel_config, f, indent=2\n",
                    "\n",
    ...
    printf"✅ Created Colab notebook: {notebook_path}"
