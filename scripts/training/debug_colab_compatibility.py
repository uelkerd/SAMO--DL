...
def check_python_version():
    ...
    printf"Python {version.major}.{version.minor}.{version.micro}"
...
def check_gpu_availability():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            printf"CUDA version: {torch.version.cuda}"
            return True
        else:
...
def check_pytorch_installation():
    ...
    try:
        ...
        if torch.cuda.is_available():
            x_cuda = x.cuda()
            y_cuda = y.cuda()
            z_cuda = torch.mmx_cuda, y_cuda
            print"✅ CUDA operations work"
        
        return True
    except Exception as e:
        printf"❌ PyTorch test failed: {e}"
        return False
...
def check_transformers_installation():
    ...
    try:
        ...
        printf"❌ Transformers test failed: {e}"
        return False
...
def check_triton_compatibility():
    ...
    try:
        ...
        if hasattrtorch, 'sparse' and hasattrtorch.sparse, '_triton_ops_meta':
            ...
            try:
                import triton
                printf"Triton version: {triton.__version__}"
                return True
            except ImportError:
                print"❌ Triton not installed"
                return False
    except Exception as e:
        printf"❌ Triton check failed: {e}"
        return False
...
def check_dataset_loading():
    ...
    try:
        ...
        print(f"✅ GoEmotions dataset loaded: {lendataset['train']} samples")
        
        # Test journal dataset
        import json
        with open'data/journal_test_dataset.json', 'r' as f:
            journal_data = json.loadf
        print(f"✅ Journal dataset loaded: {lenjournal_data} samples")
        
        return True
    except Exception as e:
        printf"❌ Dataset loading failed: {e}"
        return False
