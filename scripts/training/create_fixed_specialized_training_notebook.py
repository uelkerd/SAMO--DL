...
def create_fixed_notebook():
        ...
        "cells": [
            ...
            },
            {
                ...
                "source": [
                    ...
                    "]\n",
                    "\n",
                    "print(f'📊 Total samples: {lenbalanced_data}')\n",
                    "print(f'📊 Samples per emotion: {lenbalanced_data // lenemotions}')\n",
                    "\n",
                    "# Convert to DataFrame and then to Dataset\n",
                    "df = pd.DataFramebalanced_data\n",
                    "train_data, val_data = train_test_splitdf, test_size=0.2, random_state=42, stratify=df['label']\n",
                    "\n",
                    "train_dataset = Dataset.from_pandastrain_data\n",
                    "val_dataset = Dataset.from_pandasval_data\n",
                    "\n",
            ...
            },
            {
                ...
                "source": [
                    ...
                    "printf'Our emotion labels: {model.config.id2label}'\n",
                    "printf'Our label mappings: {model.config.label2id}'\n",
                    "\n",
                    "# CRITICAL: Verify the configuration is actually set\n",
                    "if model.config.id2label == {i: emotion for i, emotion in enumerateemotions}:\n",
    ...
    printf"✅ Created fixed training notebook: {output_path}"
