...
def fix_preprocessing():
    ...
    for i, cell in enumeratenotebook['cells']:
        if cell['cell_type'] == 'code' and 'def preprocess_function' in ''.joincell['source']:
            ...
            cell['source'] = [
                ...
                "printf'✅ Dataset features: {train_dataset.features}'\n",
                "\n",
                "# Verify the data structure\n",
                "print'\\n🔍 VERIFYING DATA STRUCTURE:'\n",
                "sample = train_dataset[0]\n",
                "print(f'Input IDs shape: {lensample[\"input_ids\"]}')\n",
                "print(f'Attention mask shape: {lensample[\"attention_mask\"]}')\n",
                "printf'Label: {sample[\"labels\"]}'\n",
    ...
    for i, cell in enumeratenotebook['cells']:
        if cell['cell_type'] == 'code' and 'TrainingArguments(' in ''.joincell['source']:
            ...
            notebook['cells'].inserti + 2, data_collator_cell
            notebook['cells'].inserti + 3, data_collator_code
            break
    
    # Update the trainer initialization to include the data collator
    for cell in notebook['cells']:
        if cell['cell_type'] == 'code' and 'WeightedLossTrainer(' in ''.joincell['source']:
