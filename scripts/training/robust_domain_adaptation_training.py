...
def verify_installation():
    ...
    try:
        ...
        printf"  Transformers: {transformers.__version__}"
        print(f"  CUDA Available: {torch.cuda.is_available()}")
        
        if torch.cuda.is_available():
            print(f"  GPU: {torch.cuda.get_device_name0}")
            print(f"  Memory: {torch.cuda.get_device_properties0.total_memory / 1e9:.1f} GB")
        ...
        printf"  ❌ Installation verification failed: {e}"
        return False
...
def setup_repository():
    ...
    def run_commandcommand: str, description: str -> bool:
        """Execute command with error handling."""
        printf"🔄 {description}..."
        try:
            result = subprocess.runcommand, shell=True, capture_output=True, text=True
            if result.returncode == 0:
                printf"  ✅ {description} completed"
                return True
            else:
                printf"  ❌ {description} failed: {result.stderr}"
                return False
        except Exception as e:
            printf"  ❌ {description} failed: {e}"
            return False
...
def safe_load_datasetdataset_name: str, config: Optional[str] = None, split: Optional[str] = None:
    """Safely load dataset with error handling."""
    try:
        from datasets import load_dataset
        if config:
            dataset = load_datasetdataset_name, config, split=split
        else:
            dataset = load_datasetdataset_name, split=split
        printf"✅ Successfully loaded {dataset_name}"
        return dataset
    except Exception as e:
        printf"❌ Failed to load {dataset_name}: {e}"
        return None

def safe_load_jsonfile_path: str:
    """Safely load JSON file with error handling."""
    try:
        with openfile_path, 'r' as f:
            data = json.loadf
        printf"✅ Successfully loaded {file_path}"
        return data
    except Exception as e:
        printf"❌ Failed to load {file_path}: {e}"
        return None

def analyze_writing_styletexts: List[str], domain_name: str -> Optional[Dict[str, float]]:
    """Analyze writing style characteristics of a domain."""
    if not texts:
        printf"⚠️ No texts provided for {domain_name}"
        return None
    
    # Filter out None or empty texts
    valid_texts = [text for text in texts if text and isinstancetext, str]
    
    if not valid_texts:
        printf"⚠️ No valid texts found for {domain_name}"
        return None
    ...
    printf"{domain_name} Style Analysis:"
    printf"  Average length: {avg_length:.1f} words"
    printf"  Personal pronouns: {personal_pronouns:.1%}"
    printf"  Reflection words: {reflection_words:.1%}"
...
def perform_domain_analysis():
    ...
    if go_texts and journal_texts:
        ...
        journal_analysis = analyze_writing_stylejournal_texts, "Journal Entries"
        
        if go_analysis and journal_analysis:
            print"\n🎯 Key Insights:"
            printf"- Journal entries are {journal_analysis['avg_length']/go_analysis['avg_length']:.1f}x longer"
            printf"- Journal entries use {journal_analysis['personal_pronouns']/go_analysis['personal_pronouns']:.1f}x more personal pronouns"
            printf"- Journal entries contain {journal_analysis['reflection_words']/go_analysis['reflection_words']:.1f}x more reflection words"
...
class FocalLoss:
    ...
    def __call__self, inputs, targets:
        ce_loss = self.F.cross_entropyinputs, targets, reduction='none'
...
class DomainAdaptedEmotionClassifier:
    ...
    def __init__self, model_name="bert-base-uncased", num_labels=None, dropout=0.3:
        ...
        printf"🏗️ Initializing DomainAdaptedEmotionClassifier with num_labels = {num_labels}"
        ...
        try:
            ...
            printf"✅ Model initialized successfully with {num_labels} labels"
            
        except Exception as e:
            printf"❌ Failed to initialize model: {e}"
            raise
    ...
    def forwardself, input_ids, attention_mask, domain_labels=None:
        try:
            ...
            printf"❌ Forward pass failed: {e}"
            raise

def safe_model_initializationmodel_name: str, num_labels: int, device: str:
    """Safely initialize model with error handling."""
    try:
        printf"🏗️ Initializing model with {model_name}..."
        
        # Initialize tokenizer
        from transformers import AutoTokenizer
        tokenizer = AutoTokenizer.from_pretrainedmodel_name
        printf"✅ Tokenizer loaded: {model_name}"
        
        # Initialize model
        model = DomainAdaptedEmotionClassifiermodel_name=model_name, num_labels=num_labels
        
        # Move to device
        import torch
        model = model.todevice
        printf"✅ Model moved to {device}"
        
        # Verify model parameters
        total_params = sum(p.numel() for p in model.parameters())
        printf"📊 Model parameters: {total_params:,}"
        
        return model, tokenizer
        
    except Exception as e:
        printf"❌ Model initialization failed: {e}"
        raise
