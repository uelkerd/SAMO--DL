    ...
    new_cells = [
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "printf'Class weights: {class_weights}'\n",
                ...
                "        self.focal_loss = FocalLossalpha=focal_alpha, gamma=focal_gamma\n",
                "        self.class_weights = class_weights\n",
                "    \n",
                "    def compute_lossself, model, inputs, return_outputs=False, num_items_in_batch=None:\n",
                ...
                "            \n",
                "            # Apply class weights\n",
                "            for i, weight in enumerateself.class_weights:\n",
                "                mask = labels == i\n",
                "                focal_loss[mask] *= weight\n",
                "            \n",
                "            loss = focal_loss.mean()\n",
                "        else:\n",
                "            loss = self.focal_losslogits, labels\n",
                "        \n",
        ...
        },
        {
            ...
            "source": [
                ...
                "model = AutoModelForSequenceClassification.from_pretrainedspecialized_model_name\n",
                "\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "printf'Precision: {results[\"eval_precision\"]:.4f}'\n",
                "printf'Recall: {results[\"eval_recall\"]:.4f}'\n",
                "\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "    with torch.no_grad():\n",
                "        outputs = model**inputs\n",
                "        predictions = torch.nn.functional.softmaxoutputs.logits, dim=-1\n",
        ...
        },
        {
            ...
            "source": [
                ...
                "print'\\n🧪 TESTING SAVED MODEL:'\n",
                "test_tokenizer = AutoTokenizer.from_pretrainedmodel_path\n",
                "test_model = AutoModelForSequenceClassification.from_pretrainedmodel_path\n",
                "\n",
                "test_input = 'I feel happy about the results!'\n",
                "test_encoding = test_tokenizertest_input, return_tensors='pt', truncation=True, padding=True\n",
                "test_encoding = {k: v.todevice for k, v in test_encoding.items()}\n",
                "\n",
                "with torch.no_grad():\n",
                "    test_outputs = test_model**test_encoding\n",
                "    test_predictions = torch.nn.functional.softmaxtest_outputs.logits, dim=-1\n",
                "    test_predicted_class = torch.argmaxtest_predictions, dim=-1.item()\n",
                "    test_confidence = test_predictions[0][test_predicted_class].item()\n",
                "\n",
                "printf'Test input: \"{test_input}\"'\n",
                "printf'Predicted emotion: {test_model.config.id2label[test_predicted_class]}'\n",
                "printf'Confidence: {test_confidence:.3f}'\n",
                "\n",
