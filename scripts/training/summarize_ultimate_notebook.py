    ...
    for cell in notebook['cells']:
        ...
        if cell['cell_type'] == 'markdown':
            ...
            if first_line.startswith'#':
                printf"   {cell_count:2d}. 📝 {first_line}"
        elif cell['cell_type'] == 'code':
            # Look for key functions/classes
            code_text = ''.joincell['source']
            if 'FocalLoss' in code_text:
                printf"   {cell_count:2d}. 🎯 Focal Loss Implementation"
            elif 'WeightedLossTrainer' in code_text:
                printf"   {cell_count:2d}. ⚖️ Weighted Loss Trainer"
            elif 'augment_text' in code_text:
                printf"   {cell_count:2d}. 📊 Data Augmentation"
            elif 'compute_metrics' in code_text:
                printf"   {cell_count:2d}. 📈 Compute Metrics"
            elif 'trainer.train()' in code_text:
                printf"   {cell_count:2d}. 🚀 Training Execution"
            elif 'model.save_pretrained' in code_text:
                printf"   {cell_count:2d}. 💾 Model Saving with Verification"
