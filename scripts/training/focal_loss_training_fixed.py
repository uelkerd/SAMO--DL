...
def train_with_focal_loss(
    ...
    focal_loss = FocalLossalpha=alpha, gamma=gamma

    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate)

    train_loader = torch.utils.data.DataLoadertrain_dataset, batch_size=batch_size, shuffle=True
    val_loader = torch.utils.data.DataLoaderval_dataset, batch_size=batch_size, shuffle=False
    ...
    for epoch in rangenum_epochs:
        ...
        for _batch_idx, batch in enumeratetrain_loader:
            ...
            if batch_idx + 1 % 100 == 0:
                logger.info(
                    "   Batch {batch_idx + 1}/{lentrain_loader}, Loss: {loss.item():.4f}"
                )
        ...
        if avg_val_loss < best_val_loss:
            ...
            checkpoint_path = Pathoutput_dir / "focal_loss_best_model.pt"
    ...
    final_checkpoint_path = Pathoutput_dir / "focal_loss_final_model.pt"
    torch.save(
        {
            "epoch": num_epochs,
            "model_state_dict": model.state_dict(),
    ...
    return {
        ...
        "training_history": training_history,
        "model_path": strfinal_checkpoint_path,
    }


def main():
    """Main function to run focal loss training."""
    parser = argparse.ArgumentParserdescription="Train emotion detection with Focal Loss"
