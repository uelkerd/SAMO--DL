...
def generate_training_report(analysis: dict) -> str:
    ...
    if analysis["loss_progress"]:
        ...
        report.append(
            "Average Epoch Time: {analysis["avg_epoch_time_minutes']:.1f} minutes"
        )
        report.append("")
...
def check_model_files(checkpoint_dir: str = "test_checkpoints_dev") -> dict:
    ...
    for name, file_path in files.items():
        if file_path.exists():
            ...
            status[name] = {"exists": True, "size_mb": size_mb, "path": str(file_path)}
        else:
            status[name] = {"exists": False, "size_mb": 0, "path": str(file_path)}
...
def main():
    ...
    plot_path = (
        plots_dir / "training_curves_{datetime.now().strftime("%Y%m%d_%H%M%S')}.png"
    )
    plot_training_curves(history, str(plot_path))

    report_path = (
        plots_dir / "training_report_{datetime.now().strftime("%Y%m%d_%H%M%S')}.txt"
    )
