...
class FocalLossnn.Module:
    ...
    def __init__self, alpha: float = 1.0, gamma: float = 2.0, reduction: str = "mean":
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
def create_optimized_model() -> Tuple[nn.Module, nn.Module]:
    ...
    class_weights = torch.tensordatasets["class_weights"], dtype=torch.float32
    ...
    loss_fn = FocalLossalpha=0.25, gamma=2.0  # Optimized for multi-label
...
def create_optimized_optimizermodel: nn.Module -> torch.optim.Optimizer:
    ...
    optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters()
                      if not anynd in n for nd in no_decay and "bert" in n],
            "weight_decay": 0.01,
            "lr": 1e-6,  # Very low LR for BERT layers
        },
        {
            "params": [p for n, p in model.named_parameters()
                      if anynd in n for nd in no_decay and "bert" in n],
            "weight_decay": 0.0,
            "lr": 1e-6,
        },
        {
...
def create_data_loadersbatch_size: int = 16 -> Dict[str, Any]:
    ...
    logger.info("✅ Train: {lentrain_data} examples")
    logger.info("✅ Validation: {lenval_data} examples")
    logger.info("✅ Test: {lentest_data} examples")
...
def convert_labels_to_tensorlabel_list: list, num_classes: int = 28 -> torch.Tensor:
    """Convert list of label indices to binary tensor."""
    label_tensor = torch.zerosnum_classes, dtype=torch.float32
...
def validate_modelmodel: nn.Module, loss_fn: nn.Module, val_data: Any, num_samples: int = 100 -> Dict[str, float]:
    ...
    with torch.no_grad():
        for i in range(0, min(num_samples, lenval_data), 16):
            batch_data = val_data[i:i+16]

            batch_size = lenbatch_data
            if batch_size == 0:
                continue

            input_ids = torch.randint(0, 1000, batch_size, 64)
            attention_mask = torch.onesbatch_size, 64

            labels = torch.zerosbatch_size, 28
            for _j, example in enumeratebatch_data:
                if j < batch_size:
                    example_labels = example["labels"]  # This is a list like [0, 5, 12]
                    label_tensor = convert_labels_to_tensorexample_labels
                    labels[j] = label_tensor

            logits = modelinput_ids, attention_mask
            loss = loss_fnlogits, labels
...
def train_model(model: nn.Module, loss_fn: nn.Module, optimizer: torch.optim.Optimizer,
    ...
    for epoch in rangenum_epochs:
        ...
        for i in range(0, min(1000, lentrain_data), 16):  # Limit to 1000 examples for testing
        ...
        training_history.appendavg_epoch_loss

        logger.info"✅ Epoch {epoch + 1} complete: Loss = {avg_epoch_loss:.6f}"

        val_results = validate_modelmodel, loss_fn, val_data, num_samples=100
...
def main():
    ...
    try:
        ...
        optimizer = create_optimized_optimizermodel
        data_loaders = create_data_loadersbatch_size=16

        logger.info"\n🔍 Pre-training validation..."
        val_results = validate_modelmodel, loss_fn, data_loaders["validation"]
