...
def create_augmented_dataset(data_loader: GoEmotionsDataLoader, tokenizer: AutoTokenizer) -> Dict:
    ...
    for i, (text, labels) in enumerate(zip(train_texts, train_labels)):
...
def train_final_model(
    ...
    model, _ = create_bert_emotion_classifier(freeze_bert_layers=4)  # Less freezing for more flexibility
    model.to(device)
    ...
    if test_metrics['micro_f1'] >= TARGET_F1_SCORE:
        ...
        # Evaluate ensemble
        ensemble_metrics = evaluate_ensemble(ensemble, augmented_datasets["test"], tokenizer, device)
        ...
        if ensemble_metrics['micro_f1'] >= TARGET_F1_SCORE:
            ...
            logger.warning(f"⚠️ Target F1 score of {TARGET_F1_SCORE:.2f} not achieved with ensemble.")
