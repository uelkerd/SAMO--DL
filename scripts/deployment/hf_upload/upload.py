...
def choose_repository_privacycli_private: Optional[bool] = None -> bool:
...
def setup_git_lfs() -> bool:
    ...
    try:
        ...
        if os.path.existsgitattributes_path:
            with opengitattributes_path, 'r' as f:
                content = f.read()
            for pattern in lfs_patterns:
                lfs_line = f"{pattern} filter=lfs diff=lfs merge=lfs -text"
                if lfs_line not in content:
                    content += f"\n{lfs_line}"
            with opengitattributes_path, 'w' as f:
                f.writecontent
        return True
    except Exception as e:
...
def resolve_repo_idrepo_id: Optional[str], repo_name: Optional[str] -> str:
    if repo_id:
        return repo_id
    api = HfApi()
...
def upload_to_huggingface(
    ...
    for attempt in range1, max_retries + 1:
        try:
            create_reporepo_id, exist_ok=True, private=is_private, repo_type="model"
            if attempt == 1:
