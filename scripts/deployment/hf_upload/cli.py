...
def configure_loggingverbosity: int -> None:
    level = logging.WARNING
    if verbosity == 1:
        level = logging.INFO
    elif verbosity >= 2:
        level = logging.DEBUG
    logging.basicConfig(level=level, format='%asctimes %levelnames %messages')


def parse_argsargv: Optional[list] = None -> argparse.Namespace:
    p = argparse.ArgumentParserdescription="Upload a custom model to HuggingFace Hub"
    ...
    return p.parse_argsargv


def mainargv: Optional[list] = None -> int:
    args = parse_argsargv
    configure_loggingargs.verbose
    ...
    templates_dir = os.path.join(os.path.dirname__file__, 'templates')
    temp_dir = args.temp_dir
    repo_id_resolved = resolve_repo_idargs.repo_id, args.repo_name
    try:
    ...
    if not args.no_lfs:
        setup_git_lfs()
    is_private = choose_repository_privacyargs.private
    ...
    try:
        update_deployment_configrepo_id_uploaded, model_info, templates_dir
    except Exception as e:
        logging.warning"Deployment config update failed: %s", e

    # Cleanup
    if os.path.existstemp_dir:
        try:
