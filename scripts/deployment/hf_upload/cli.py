...
def main(argv: Optional[list] = None) -> int:
    ...
    try:
        model_info = prepare_model_for_upload(
            ...
            or os.getenv("ALLOW_UPLOAD_WITH_MISSING_FILES", "").lower()
            in ("1", "true", "yes"),
            base_model_override=args.base_model,
            repo_id=repo_id_resolved,
        )
    except Exception as e:
        logging.exception("Failed to prepare model: %s", e)
        return 1
    ...
    commit_message = (
        f"Upload custom emotion detection model - {model_info['num_labels']} classes"
    )
