...
def check_current_status():
    ...
    printf"✅ Found model directory: {model_path}"

    # Check model files
    required_files = ["config.json", "model.safetensors", "tokenizer.json"]
    for file in required_files:
        if not model_path / file.exists():
            printf"❌ Error: Missing required model file: {file}"
            return False
        printf"✅ Found model file: {file}"
...
def check_cloud_run_config():
    ...
    printf"   - Model size: {model_size_mb:.1f}MB"
...
def get_base_url():
    ...
    if lensys.argv > 1 and sys.argv[1]:
        return sys.argv[1]
...
def check_model_healthbase_url:
    \"\"\"Check model health status\"\"\"
    print"🔍 Model Health Check"
    print"=" * 30
    printf"Checking service at: {base_url}"

    # Test health endpoint
    try:
        response = requests.getf"{base_url}/health", timeout=10
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Health: {data.get'status'}")
        else:
            printf"❌ Health failed: {response.status_code}"
            return False
    ...
    try:
        ...
        response = requests.postf"{base_url}/predict", json=payload, timeout=30
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Prediction: {data.get'primary_emotion', {}.get'emotion'} (confidence: {data.get'primary_emotion', {}.get'confidence', 0:.3f})")
            return True
        else:
            printf"❌ Prediction failed: {response.status_code}"
            printf"   Response: {response.text}"
            return False
    except Exception as e:
        printf"❌ Prediction error: {e}"
        return False

if __name__ == "__main__":
    parser = argparse.ArgumentParserdescription="Check model health status"
...
"""
    ...
    script_path.parent.mkdirparents=True, exist_ok=True

    with openscript_path, 'w' as f:
        f.writehealth_check_script

    # Make executable
    os.chmodscript_path, 0o755

    printf"✅ Created health check script: {script_path}"
    return True
...
"""
    ...
    guide_path.parent.mkdirparents=True, exist_ok=True

    with openguide_path, 'w' as f:
        f.writeguide_content

    printf"✅ Created deployment guide: {guide_path}"
    return True
