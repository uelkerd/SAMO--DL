...
class SecurityDeploymentFix:
    ...
    def run_commandself, command: List[str], check: bool = True -> subprocess.CompletedProcess:
        ...
        try:
            ...
            result = subprocess.runsanitized_command, capture_output=True, text=True, check=check
            if result.stdout:
    ...
    def verify_static_files_existself:
        ...
        for file_path in required_files:
            if not file_path.exists():
                missing_files.append(strfile_path)

        if missing_files:
            raise FileNotFoundError(f"Missing required static files: {', '.joinmissing_files}")

        self.log"✅ All static files verified"

    def create_secure_requirementsself:
...
'''
        ...
        with opencloudbuild_path, 'w' as f:
            f.writecloudbuild_content
    ...
    def test_deploymentself:
        ...
        try:
            result = self.run_command([
                ...
                '--region', REGION,
                '--format', 'valuestatus.url'
            ])
            service_url = result.stdout.strip()
        except Exception as e:
            self.logf"Failed to get service URL: {e}", "ERROR"
            return False
        ...
        try:
            response = requests.getf"{service_url}/health", timeout=30
            if response.status_code != 200:
                self.logf"Health check failed: {response.status_code}", "ERROR"
                return False
            self.log"✅ Health check passed"
        except Exception as e:
            self.logf"Health check failed: {e}", "ERROR"
            return False

        # Test security headers
        try:
            response = requests.getf"{service_url}/", timeout=30
            ...
            if missing_headers:
                self.logf"Missing security headers: {missing_headers}", "WARNING"
            else:
                self.log"✅ Security headers present"

        except Exception as e:
            self.logf"Security headers test failed: {e}", "ERROR"
            return False

        # Test API key protection
        try:
            response = requests.getf"{service_url}/model_status", timeout=30
            if response.status_code != 401:
                self.log"API key protection not working", "ERROR"
                return False
            self.log"✅ API key protection working"
        except Exception as e:
            self.logf"API key test failed: {e}", "ERROR"
            return False

        # Test rate limiting
        try:
            responses = []
            for i in range105:  # Exceed rate limit
            ...
            self.logf"Rate limiting test failed: {e}", "ERROR"
            return False

        self.log"✅ All security tests passed"
        return True

    def cleanup_old_deploymentself:
    ...
    def runself:
        ...
        try:
            ...
            self.logf"❌ Security deployment fix failed: {e}", "ERROR"
            return False
