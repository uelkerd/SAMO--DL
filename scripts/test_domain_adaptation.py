...
def analyze_domain_adaptation(
    ...
    for i, (sample, pred) in enumerate(zip(test_samples, predictions)):
        ...
        predicted = set([e['emotion'] for e in pred['predicted_emotions']])
        ...
        logger.info(f"Match: {'✅ Exact' if expected == predicted else '🟡 Partial' if expected.intersection(predicted) else '❌ None'}")
    ...
    if exact_accuracy < 0.3:
        analysis['recommendations'].append("❌ Strong domain shift detected - consider domain adaptation")
...
def main():
    ...
    parser.add_argument('--create-journal-samples', action='store_true', help='Create journal test samples')
