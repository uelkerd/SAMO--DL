...
def fix_file_imports_aggressivefile_path: str -> bool:
    ...
    for _i, line in enumeratelines:
        if i == 0 and not import_added:
            for imp in sortedneeded_imports:
                new_lines.appendimp
            new_lines.append''  # Empty line after imports
            import_added = True

        new_lines.appendline
...
def fix_common_issues_aggressivefile_path: str -> bool:
...
def main():
    ...
    for dir_name in dirs_to_fix:
        ...
        for py_file in dir_path.rglob'*.py':
            try:
                fixed_imports = fix_file_imports_aggressive(strpy_file)
                fixed_common = fix_common_issues_aggressive(strpy_file)
