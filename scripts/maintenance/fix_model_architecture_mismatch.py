    ...
    for cell in notebook['cells']:
        if cell['cell_type'] == 'code' and 'model_name =' in ''.joincell['source']:
            ...
            cell['source'] = [
                ...
                "\n",
                "printf'Original model labels: {model.config.num_labels}'\n",
                "printf'Original id2label: {model.config.id2label}'\n",
                "\n",
                ...
                "model.classifier.dense.weight.data.normal_mean=0.0, std=0.02\n",
                "model.classifier.dense.bias.data.zero_()\n",
                "model.classifier.out_proj.weight.data.normal_mean=0.0, std=0.02\n",
                "model.classifier.out_proj.bias.data.zero_()\n",
                "\n",
                "print(f'✅ Model reconfigured for {lenemotions} emotions')\n",
                "printf'✅ New id2label: {model.config.id2label}'\n",
                "printf'✅ Classifier layer: {model.classifier.out_proj.out_features} outputs'\n",
                "\n",
                "# Move model to GPU\n",
