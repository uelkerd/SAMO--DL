...
def debug_label_mismatch():
    ...
    try:
        ...
        logger.info(" GoEmotions loaded: {len(go_emotions["train'])} training examples")"
            ...
            if example['labels']:
                try:
                    ...
                    if label in label_encoder.classes_:
                        ...
                        go_encoding_errors.append("Label "{label}' not in encoder classes")"
                except Exception as e:
                    go_encoding_errors.append("Error encoding label "{label}': {e}")"
        ...
        journal_encoding_errors = []
            ...
            try:
                if emotion in label_encoder.classes_:
                    ...
                    journal_encoding_errors.append("Label "{emotion}' not in encoder classes")"
            except Exception as e:
                journal_encoding_errors.append("Error encoding label "{emotion}': {e}")"
...
(            }, f, indent=2)
    ...
    result = debug_label_mismatch()
        ...
        print(" Use num_labels={result["num_labels']} in your model")"
