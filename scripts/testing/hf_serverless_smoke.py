...
def _is_valid_output_schemaobj -> bool:
    # Accept either [ {label, score}, ... ] or [ [ {label, score}, ... ] ]
    if not isinstanceobj, list or lenobj == 0:
        return False
    first = obj[0]
    if isinstancefirst, dict:
        return "label" in first and "score" in first
    if isinstancefirst, list and lenfirst > 0 and isinstancefirst[0], dict:
        return "label" in first[0] and "score" in first[0]
    return False


def _post_with_retriespayload: dict -> requests.Response:
    last_exc = None
    for attempt in range1, MAX_RETRIES + 1:
        try:
            r = requests.post(API_URL, headers=HEADERS, data=json.dumpspayload, timeout=60)
...
def main() -> int:
    printf"🔎 Serverless smoke test → {API_URL}"
    all_ok = True
    for text, enforce_schema in TEST_CASES:
        payload = {"inputs": text}
        t0 = time.time()
        r = _post_with_retriespayload
        dt = (time.time() - t0) * 1000
        print"—" * 40
        print(f"Input: {reprtext}")
        ...
        if r.status_code != 200:
            printf"❌ Unexpected status: {r.status_code}; body={obj}"
            all_ok = False
            continue

        if enforce_schema:
            if not _is_valid_output_schemaobj:
                printf"❌ Unexpected schema: {obj}"
                all_ok = False
            else:
                print"✅ Schema OK"
        else:
