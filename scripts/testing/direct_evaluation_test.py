...
def test_direct_evaluation():
    ...
    with torch.no_grad():
        ...
        logger.info(
            ...
            "{probabilities.min().item():.4f}/{probabilities.max().item():.4f}/"
            "{probabilities.mean().item():.4f}",
        )
        ...
        logger.info(
            "  - Match expected: "
            "{'✅' if predictions.sum().item() == expected_predictions else '❌'}",
        )
        ...
        logger.info(
            ...
            "{100*samples_with_zero/samples_per_batch:.1f}%",
        )
