...
def generate_journal_content(topic: str, emotion: str) -> str:
    ...
    content = template.format(
        topic=topic, emotion=emotion, emotion_context=emotion_context, reflection=reflection
    )
...
def create_journal_test_dataset(
    num_entries: int = 150, num_users: int = 10, days_back: int = 90
) -> List[Dict[str, Any]]:
...
def create_dataset_summary(entries: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "date_range": {"start": min(df["created_at"]), "end": max(df["created_at"])},
...
def main():
    ...
    entries = create_journal_test_dataset(
        num_entries=150, num_users=10, days_back=90  # Exceeds the 100+ requirement
    )
