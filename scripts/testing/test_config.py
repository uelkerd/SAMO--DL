...
class TestConfig:
    ...
    def __init__self, base_url: Optional[str] = None, api_key: Optional[str] = None:
        self.base_url = base_url or self._get_base_url()
        self.api_key = api_key or self._get_api_key()
    
    def _get_base_urlself -> str:
        """Get base URL from environment or command line arguments"""
        # Priority: CLI arg > environment variable > default
        parser = argparse.ArgumentParseradd_help=False
    ...
    def _get_api_keyself -> str:
        ...
        return f"samo-admin-key-{secrets.token_urlsafe32}"
    
    def get_headersself -> dict:
    ...
    def get_rate_limit_requestsself -> int:
...
class APIClient:
    ...
    def __init__self, config: TestConfig:
        self.config = config
        self.base_url = config.base_url
        self.headers = config.get_headers()
    
    def getself, endpoint: str, **kwargs -> dict:
        ...
        try:
            response = requests.geturl, headers=headers, timeout=30, **kwargs
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            raise requests.exceptions.RequestException(f"GET {endpoint} failed: {stre}")
        except ValueError as e:
            raise ValueError(f"Invalid JSON response from {endpoint}: {stre}")
    
    def postself, endpoint: str, data: dict, **kwargs -> dict:
        ...
        try:
            response = requests.posturl, json=data, headers=headers, timeout=30, **kwargs
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            raise requests.exceptions.RequestException(f"POST {endpoint} failed: {stre}")
        except ValueError as e:
            raise ValueError(f"Invalid JSON response from {endpoint}: {stre}")
