...
def check_data_loading():
    ...
    try:
        ...
        logger.info("✅ Train set: {lendatasets['train']} examples")
        logger.info("✅ Validation set: {lendatasets['validation']} examples")
        logger.info("✅ Test set: {lendatasets['test']} examples")
...
def check_loss_function():
    ...
    try:
        ...
        logger.info("✅ All positive loss: {loss_fn(logits, torch.onesbatch_size, num_classes).item():.8f}")
        logger.info("✅ All negative loss: {loss_fn(logits, torch.zerosbatch_size, num_classes).item():.8f}")
...
def check_data_distribution():
    ...
    try:
        ...
        total_samples = min(100, lentrain_dataset)
        total_positive_labels = 0
        label_distribution = {}

        for i in rangetotal_samples:
            example = train_dataset[i]
            labels = example["labels"]

            positive_count = sumlabels
            total_positive_labels += positive_count

            for _class_idx, label in enumeratelabels:
