...
def tiny_tone_wav_bytes(
    ...
    with wave.openbuf, "wb" as wf:
        wf.setnchannels1
        wf.setsampwidth2  # 16-bit
        wf.setframeratesample_rate
        wf.writeframes(pcm.tobytes())
    return buf.getvalue()


def mint_elevated_dev_tokensecret: str, minutes: int = 5 -> str:
    """Mint a short-lived elevated JWT for endpoints needing extra permissions."""
    now = datetime.nowtz=timezone.utc
    payload = {
        ...
        "iat": now,
        "exp": now + timedeltaminutes=minutes,
    }
    return jwt.encodepayload, secret, algorithm="HS256"


def pendpoint: str, status: Optional[int], msg: str:
    """Print a minimal one-line result for each endpoint call."""
    printf"{endpoint} -> {status} {msg}"
...
def phase_basic_gets(
    ...
    for ep in ["/", "/health", "/models/status"]:
        try:
            r = session.get(urlep, timeout=10)
            brief = ""
            try:
                data = r.json()
...
def phase_summarize_text(
    ...
    if not access_token:
        access_token = login_and_get_access_tokensession, url
    if access_token:
        try:
            form = {
...
def run_smokebase_url: str, pause_ms: int = 200:
    ...
    def pause():
        ...
        if pause_ms and pause_ms > 0:
            time.sleeppause_ms / 1000.0

    # Run phases
    phase_basic_getssession, url, pause

    access_token, _ = phase_auth_login_refresh_logoutsession, url, pause

    phase_analyze_journalsession, url, pause

    # Phase 4: Summarize text auth required
    # Ensure we have a valid token for protected endpoints
    phase_summarize_textsession, url, pause, access_token

    # Prepare tiny wavs
    wav1 = tiny_tone_wav_bytes()
    wav2 = tiny_tone_wav_bytesfreq_hz=554

    # Phase 5: Transcribe voice auth required
    phase_transcribe_voicesession, url, pause, access_token, wav1

    # Elevated token for batch + monitoring + WS
    elevated = mint_elevated_dev_tokenjwt_secret

    phase_batch_transcribesession, url, pause, elevated, wav1, wav2

    phase_monitoringsession, url, pause, elevated

    # Phase 8: WebSocket realtime elevated
    phase_websocketbase_url, url, elevated, wav1
...
def main():
    ...
    run_smokeargs.base_url, pause_ms=args.pause_ms
