...
def debug_evaluation_step_by_step():
    ...
    trainer = EmotionDetectionTrainerdev_mode=True, batch_size=128, num_epochs=1
    ...
    trainer.model.eval()
    with torch.no_grad():
        outputs = trainer.modelinput_ids, attention_mask
    ...
    for threshold in thresholds:
        ...
        logger.info(
            "    - Samples with 0 predictions: {(predictions_before_fallback.sumdim=1 == 0).sum().item()}"
        )
        logger.info(
            "    - Samples with >0 predictions: {(predictions_before_fallback.sumdim=1 > 0).sum().item()}"
        )

        samples_needing_fallback = predictions_before_fallback.sumdim=1 == 0
        ...
        if num_samples_needing_fallback > 0:
            ...
            for sample_idx in rangepredictions_after_fallback.shape[0]:
        ...
        logger.info(
            "    - Samples with 0 predictions: {(predictions_after_fallback.sumdim=1 == 0).sum().item()}"
        )
