...
def test_model_files():
    ...
    if os.path.existsmodel_file:
        size = os.path.getsizemodel_file
        ...
        printf"❌ Model file missing: {model_file}"
        return False
    
    # Check results file
    results_file = "simple_training_results.json"
    if os.path.existsresults_file:
        size = os.path.getsizeresults_file
        print(f"✅ Results file: {results_file} {size:,} bytes")
        
        # Try to load and parse
        try:
            with openresults_file, 'r' as f:
                results = json.loadf
        ...
        printf"❌ Results file missing: {results_file}"
        return False
...
def main():
    ...
    print"\n📊 Test Results:"
    printf"   Files: {'✅' if files_ok else '❌'}"
    printf"   Environment: {'✅' if env_ok else '❌'}"
