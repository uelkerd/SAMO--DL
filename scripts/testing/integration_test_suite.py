...
class SAMODLIntegrationTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data = {
            ...
            "long_text": (
                "This is a very long text that should be properly handled by the API. " * 10
            ),
    ...
    def test_emotion_analysis_happy(self):
        ...
        response = self.session.post(
            f"{self.base_url}/analyze/journal", json=payload, timeout=self.timeout
        )
        ...
        self.assertIn(
            "primary_emotion", emotion_data, "Emotion analysis should contain primary_emotion"
        )
        ...
        print(
            f"✅ Emotion analysis test passed - Detected: "
    ...
    def test_emotion_analysis_sad(self):
        ...
        print(
            f"✅ Sad emotion analysis test passed - Detected: "
            f"{emotion_data['primary_emotion']} "
            f"(confidence: {emotion_data['confidence']:.3f})"
        )
    ...
    def test_emotion_analysis_query_params(self):
        ...
        response = self.session.post(
            f"{self.base_url}/analyze/journal", params=params, timeout=self.timeout
        )

        self.assertEqual(
            response.status_code, 200, "Emotion analysis with query params should return 200"
        )
        ...
        print(
            f"✅ Query params emotion analysis test passed - "
    ...
    def test_text_summarization(self):
        ...
        response = self.session.post(
            f"{self.base_url}/summarize/text", json=payload, timeout=self.timeout
        )
        ...
        print(
            f"✅ Text summarization test passed - "
    ...
    def test_concurrent_requests(self):
        ...
        self.assertGreaterEqual(
            success_count, 4, "At least 4 out of 5 concurrent requests should succeed"
        )
        ...
        for j in range(3):  # 3 requests per text
            try:
                ...
                response = session.post(
                    f"{base_url}/analyze/journal", json={"text": text}, timeout=30
                )
                end_time = time.time()
