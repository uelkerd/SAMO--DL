...
def check_gpu_setup() -> Dict[str, any]:
    ...
    if torch.cuda.is_available():
        ...
        if memory_total < 8e9:  # Less than 8GB
            gpu_info['recommendations'].append("Consider using mixed precision training (fp16) to save memory")
            gpu_info['recommendations'].append("Reduce batch size if encountering OOM errors")
        
        if "T4" in device_name or "V100" in device_name:
            gpu_info['recommendations'].append("Tensor Core support available - use mixed precision for 2x speedup")
    ...
    for model_type in ['pytorch', 'onnx']:
        if model_type in results:
            ...
            if p95_latency <= target_latency:
                assessment[model_type] = f"✅ MEETS TARGET ({p95_latency:.1f}ms ≤ {target_latency}ms)"
            elif p95_latency <= target_latency * 1.2:  # Within 20%
                assessment[model_type] = f"⚠️  CLOSE TO TARGET ({p95_latency:.1f}ms vs {target_latency}ms)"
            else:
                assessment[model_type] = f"❌ EXCEEDS TARGET ({p95_latency:.1f}ms > {target_latency}ms)"
...
def main():
    ...
    parser.add_argument('--target-latency', type=float, default=500.0, help='Target P95 latency (ms)')
